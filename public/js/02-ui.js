...
// ---------- motion utilities ----------
const reducedMotion = () => "matchMedia" in window && matchMedia("(prefers-reduced-motion: reduce)").matches;
...
function btnBusy(btn, label, { ghost = false } = {}) {
    ...
    if (busyBtn._busyRestore)
        return busyBtn._busyRestore; // already working — don't stack
...
function wireArtZoom(artEl) {
    ...
    const lift = (e) => { touches.delete(e.pointerId); if (touches.size < 2)
        pinchBase = 0; };
...
function closeAgentLoginModal(ov) {
    ...
    try {
        if (ov._onResize)
            window.removeEventListener("resize", ov._onResize);
    }
    catch { }
    try {
        if (ov._onKey)
...
async function openAgentLoginModal(agentName) {
        ...
        switch (msg.t) {
            ...
            case "busy":
                ...
                if (typeof toast === "function")
                    toast("Another login is already running — try again in a moment.");
                closeAgentLoginModal(ov);
                break;
            case "error":
    ...
    ws.onerror = () => {
        if (!ov._failed)
            markFailed("Connection error — make sure the server is reachable, then try again.");
    };
    ws.onclose = () => {
