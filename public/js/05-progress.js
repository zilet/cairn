...
async function renderEndurance() {
    ...
    if (token !== pollToken || !view.querySelector("#endBody"))
        return;
...
function paintEnduranceBody(end, prs, goal, compliance, settings, runPlan) {
    ...
    const hasWeek = isProgressRecord(end) && (progressNumber(endRow.week_km) > 0 ||
    ...
    if (!hasWeek && !hasPRs) {
        ...
        if (syncHtml && typeof wireCardioSync === "function")
            wireCardioSync(body, () => renderEndurance());
        return;
    }
    ...
    if (isProgressRecord(end)) {
        ...
        if (endRow.week_moving_min != null)
            heroStats.push(["moving min · wk", Math.round(progressNumber(endRow.week_moving_min))]);
        if (endRow.longest_km != null)
            heroStats.push(["longest · km", endRow.longest_km, { text: true }]);
        else if (endRow.longest_min != null)
            heroStats.push(["longest · min", Math.round(progressNumber(endRow.longest_min)), { text: true }]);
    }
    ...
    if (isProgressRecord(end) && (endRow.longest_km != null || endRow.longest_min != null)) {
        ...
        if (endRow.longest_min != null)
            lbits.push(`${Math.round(progressNumber(endRow.longest_min))} min`);
