...
function tovSaveSnapshot(data) {
    try {
        sessionStorage.setItem(TOV_SNAP_KEY, JSON.stringify(data));
    }
    catch { /* quota — skip */ }
}
function tovLoadSnapshot() {
    try {
        const parsed = JSON.parse(sessionStorage.getItem(TOV_SNAP_KEY) || "null");
        return parsed && typeof parsed === "object" && "balance" in parsed ? parsed : null;
    }
    catch {
        return null;
    }
}
async function tovFetch() {
    const grab = (path) => api(path).catch(() => null);
...
async function renderTrainOverview() {
    headerTitle.textContent = "Train";
    state.progressSeg = "overview";
    const token = ++tovToken;
    if (!tovData)
        tovData = tovLoadSnapshot();
    if (tovData)
        paintTrainOverview(tovData);
    else
        view.innerHTML = segSkeleton("overview", PROGRESS_SEG, 3);
    const fresh = await tovFetch();
    if (token !== tovToken || state.tab !== "progress" || state.progressSeg !== "overview")
        return;
    const changed = JSON.stringify(fresh) !== JSON.stringify(tovData);
    tovData = fresh;
    tovSaveSnapshot(fresh);
    if (changed || !document.querySelector(".tov-mast, .tov-empty"))
        paintTrainOverview(fresh);
}
// ---- data folding -------------------------------------------------------------
function tovGroupKey(value) {
    return String(value || "").toLowerCase().trim();
}
function tovFoldRows(data) {
    const balance = new Map();
    for (const g of CairnProgressData.rows(data.balance?.groups))
        balance.set(tovGroupKey(g.group), g);
    const traj = new Map();
    for (const g of CairnProgressData.rows(data.trajectory?.groups))
        traj.set(tovGroupKey(g.group), g);
    const load = new Map();
    for (const g of CairnProgressData.rows(data.load?.groups))
        load.set(tovGroupKey(g.group), g);
    const keys = new Set([...TOV_GROUP_ORDER, ...balance.keys(), ...traj.keys()]);
    ...
    const attention = { due: 0, recover: 1, high: 2, ok: 3, none: 4 };
    const anatomical = (g) => { const i = TOV_GROUP_ORDER.indexOf(g); return i < 0 ? 99 : i; };
    rows.sort((a, b) => (attention[a.tone] - attention[b.tone]) || (anatomical(a.group) - anatomical(b.group)));
    return rows;
}
// ---- the figure ---------------------------------------------------------------
function tovEllipse(shape, attrs) {
...
function tovFigureSvg(side, tones) {
    const lib = tovFigureLib();
    if (lib)
        return lib.figureSvg(side, tones, { pulseDue: true, dataAttrs: true });
    return tovFigureSvgFallback(side, tones);
}
function tovFigureSvgFallback(side, tones) {
...
// ---- section renderers ----------------------------------------------------------
function tovHeadline(data, rows) {
...
function tovMastHtml(data, rows) {
    const stats = data.stats || {};
    const done = CairnProgressData.number(stats.week_done);
    const planned = CairnProgressData.number(stats.week_planned);
    ...
    const stat = (n, l, cu) => `<div class="stat"><div class="stat-n"${cu != null ? ` data-cu="${cu}"` : ""}>${escHtml(n)}</div><div class="stat-l">${escHtml(l)}</div></div>`;
    const strip = [
...
function tovVerdictChip(row) {
    if (row.verdict === "advancing")
        return `<span class="tov-chip tov-chip-adv">advancing ↗</span>`;
    if (row.verdict === "stalling")
        return `<span class="tov-chip tov-chip-stall">stalling</span>`;
    if (row.verdict === "building")
        return `<span class="tov-chip">building</span>`;
    return "";
}
function tovBandBar(row) {
...
function tovRowNote(row) {
    const parts = [];
    if (row.sets > 0)
        parts.push(`${row.sets} set${row.sets === 1 ? "" : "s"} this week`);
    if (row.band === "productive")
        parts.push("in the productive range");
    else if (row.band === "high")
        parts.push("above the productive range");
    else if (row.tone === "due")
        parts.push(row.sets > 0 ? "room for more" : "not trained lately");
    if (row.loadNote)
        parts.push(row.loadNote);
...
function tovMapHtml(rows) {
    const tones = {};
    for (const row of rows)
        if (row.tone !== "none")
            tones[row.group] = row.tone;
    const legend = [
...
function tovCapitalize(value) {
    return value ? value.charAt(0).toUpperCase() + value.slice(1) : value;
}
function tovFocusHtml(data) {
    const focus = data.focus || {};
    const lead = (focus.lead || null);
    if (!focus.available || !lead || !lead.title)
        return "";
    const move = lead.move ? `<div class="tov-focus-move">${escHtml(lead.move)}</div>` : "";
    const retest = (focus.retest || null);
...
function tovMovesHtml(data) {
    const moves = CairnProgressData.rows(data.adjustments).slice(0, 3);
    if (!moves.length)
        return "";
...
function tovSessionsHtml(data) {
    const sessions = CairnProgressData.rows(data.sessions);
    if (!sessions.length)
        return "";
...
// ---- paint ----------------------------------------------------------------------
function paintTrainOverview(data) {
    ...
    wireSeg(PROGRESS_HANDLERS);
    runCountUps(view);
    view.querySelectorAll("[data-tovgo]").forEach((el) => el.addEventListener("click", () => {
    ...
    view.querySelectorAll(".tov-map [data-group]").forEach((el) => {
        el.style.cursor = "pointer";
        el.addEventListener("click", () => {
            const group = el.getAttribute("data-group") || "";
            const row = group ? view.querySelector(`.tov-row[data-group="${group}"]`) : null;
            if (!row)
                return;
...
if (typeof window !== "undefined")
    Object.assign(window, { renderTrainOverview });
})();
