...
function openMealSheet(current, di, mi) {
    ...
    s.addEventListener("click", (e) => { if (e.target === s)
        closeMealSheet(); });
...
function recipeOpOpts(current, dayLabel, di, mi, key) {
    ...
    return {
        ...
        render: (r) => {
            ...
            if (plan.parsed)
                current.parsed = mealParsed(plan.parsed);
            else {
            ...
            if (!row.cached)
                swrInvalidate(MEALS_KEY); // a freshly written recipe changed the plan
...
function reconnectRecipe(job) {
    ...
    if (!current || !mealPlanDays(current).length)
        return null;
    const di = mealPlanDays(current).findIndex((d) => String(d?.day ?? "").trim().toLowerCase() === dayLabel.trim().toLowerCase());
    if (di < 0 || !Number.isFinite(mi))
        return null;
    const key = `${planId}:${di}:${mi}`;
    const wrap = document.querySelector(`.sheet[data-key="${key}"] [data-recipe]`);
    if (!wrap)
        return null; // the sheet isn't open — a re-open will show the cached recipe
    wrap.innerHTML = recipeLoadingHtml();
    ...
    const clear = () => { stop(); const h = document.querySelector(o.anchor); if (h) {
        h.classList.remove("is-thinking", "is-thinking--determinate");
        h.style.removeProperty("--frac");
    } };
    return {
        guard: o.guard,
        onDone: (result) => { clear(); if (o.isFail(result))
...
async function loadDayFuel(token) {
    ...
    try {
        d = await api("/nutrition/day" + qs);
    }
    catch {
        slot.innerHTML = "";
        return;
    }
    if (token !== pollToken || !view.querySelector("#dayFuelSlot"))
        return;
    ...
    const day = d;
    state._dayFuel = day;
    slot.innerHTML = dayFuelHtml(day);
    runCountUps(slot);
    slot.querySelectorAll("[data-fooditem]").forEach((row) => row.addEventListener("click", () => openFoodEdit(Number(row.dataset.fooditem), row)));
    const ask = slot.querySelector("#dayFuelAsk");
    if (ask)
        ask.addEventListener("click", () => gotoChatWith("How's my eating shaping up today, and does it fit my goal?"));
}
    ...
    openDetailFrom(fromEl, () => {
      ...
      <div class="detail-actions">
        ...
        el.querySelector("#fedSave")?.addEventListener("click", async () => {
            const summary = el.querySelector("#fedSummary");
            const meal = el.querySelector("#fedMeal");
            const kcal = el.querySelector("#fedKcal");
            const protein = el.querySelector("#fedProtein");
            const carbs = el.querySelector("#fedCarbs");
            const fat = el.querySelector("#fedFat");
            const body = {
