...
(() => {
    ...
    function endDraftCardHtml(proposal) {
    ...
    function enduranceRecord(value) {
    ...
    async function renderPlanEndurance() {
        ...
        try {
            [goal, compliance, plan, settings] = await Promise.all([
                ...
                api("/plan").catch(() => []),
                api("/settings").then((response) => enduranceRecord(response).settings || null).catch(() => null),
            ]);
        }
    ...
    function renderEnduranceDraftResult(proposal) {
        ...
        if (!status || !draftWrap)
            return;
        const p = enduranceRecord(proposal);
