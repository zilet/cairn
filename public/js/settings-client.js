...
/** @typedef {{ relTime?: (value: string) => string, absDate?: (value: string) => string }} SettingsDateFns */
/** @typedef {{ updateCheckEnabled: boolean }} SettingsUpdateOptions */
...
function garminStatusLine(settings, syncing, options = {}) {
  ...
  const s = settings && typeof settings === "object" ? /** @type {Record<string, unknown>} */ (settings) : {};
  const at = s.garmin_last_sync_at;
  const raw = String(s.garmin_last_sync_status || "");
...
function agentHealthCard(stats) {
  const st = stats && typeof stats === "object" ? /** @type {Record<string, unknown>} */ (stats) : null;
  ...
  const byAgent = Array.isArray(st.by_agent) ? st.by_agent : [];
  const rows = byAgent.filter((a) => a && a.agent).map((a) => {
...
function agentOpLabel(op) {
  ...
  if (Object.hasOwn(SETTINGS_AGENT_OP_LABELS, key)) {
    return SETTINGS_AGENT_OP_LABELS[key];
  }
...
function agentActivityCard(stats, options = {}) {
  const st = stats && typeof stats === "object" ? /** @type {Record<string, unknown>} */ (stats) : {};
...
function noticedCard(data, options = {}) {
  ...
  const rows = Array.isArray(d.learnings) ? d.learnings : [];
  if (!rows.length) return "";
  const items = rows.slice(0, 8).map((item) => {
 ...
 * @param {SettingsUpdateOptions} options
 */
function updateCardHtml(status, options) {
  const st = status && typeof status === "object" ? /** @type {Record<string, unknown>} */ (status) : null;
  const current = escHtml(String((st && st.current) || "—"));
  ...
  if (st.update_available && st.latest) {
    ...
    return `<div class="sess-line"><b>v${escHtml(String(st.latest))} is available</b> — you're on v${current}.${checked}</div>
