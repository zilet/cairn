...
function renderInsightCard(slot, ins) {
    ...
    slot.querySelectorAll("[data-ifb]").forEach((b) => b.addEventListener("click", () => insightFeedback(slot, ins, b.dataset.ifb)));
...
function renderWeeklyCard(slot, ins) {
    ...
    slot.querySelectorAll("[data-ifb]").forEach((b) => b.addEventListener("click", () => insightFeedback(slot, ins, b.dataset.ifb, ".weekly-card")));
    const whyBtn = slot.querySelector("[data-iwhy]");
...
function maybeGenerateWeekly() {
    ...
    if (!(dow === 0 || dow === 5 || dow === 6))
        return; // weekend reflection only
    const last = Number(localStorage.getItem("cairn:lastWeeklyGen") || 0);
    if (Date.now() - last < 6 * 24 * 3600 * 1000)
        return;
    const burnGate = () => { try {
        localStorage.setItem("cairn:lastWeeklyGen", String(Date.now()));
    }
    catch { } };
