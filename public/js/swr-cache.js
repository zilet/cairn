...
/** @template T @typedef {{ data: T, ts: number }} SwrEntry */
/** @template T @typedef {{ data: T, fresh: boolean }} SwrPeek */
/** @typedef {{ changed: boolean }} SwrUpgradeMeta */
/** @template T @typedef {{ key?: string, freshFor?: number, onUpgrade?: (data: T, meta: SwrUpgradeMeta) => void }} CachedApiOptions */
/** @template T @typedef {{ key?: string, path?: string, peek?: SwrPeek<T> | null, render?: (data: T, meta: { warm: boolean }) => void, token?: unknown, freshFor?: number, tab?: string | null }} PaintSwrOptions */
...
/** @type {Map<string, SwrEntry<unknown>>} */
...
function _swrLsGet(key) {
  ...
  try {
    ...
    if (o && typeof o === "object" && "data" in o) return /** @type {SwrEntry<unknown>} */ (o);
  } catch {}
  return null;
}
 ...
 * @template T
 * @param {string} key
 * @param {number} [freshFor]
 * @returns {SwrPeek<T> | null}
 */
function peekCached(key, freshFor = 60000) {
  ...
  return /** @type {SwrPeek<T>} */ ({ data: entry.data, fresh: age < freshFor });
}
 ...
 * @param {Path} path
 * @param {CachedApiOptions<import("../../src/contracts/client.js").ClientApiResponse<Path>>} [options]
 * @returns {Promise<import("../../src/contracts/client.js").ClientApiResponse<Path>>}
 */
function cachedApi(path, options = {}) {
  const { key, freshFor = 60000, onUpgrade } = options;
  const k = key || path;
  const prior = /** @type {SwrPeek<import("../../src/contracts/client.js").ClientApiResponse<Path>> | null} */ (
    peekCached(k, freshFor)
  );
  return api(path)
    .then((data) => {
...
/**
 * @template {string} Path
 * @param {PaintSwrOptions<import("../../src/contracts/client.js").ClientApiResponse<Path>> & { path?: Path }} [options]
 * @returns {Promise<import("../../src/contracts/client.js").ClientApiResponse<Path> | void>}
 */
