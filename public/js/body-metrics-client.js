...
(() => {
    ...
    function logForm(sites) {
        const today = new Date().toISOString().slice(0, 10);
        const inputs = sites
            .map((s) => `<label class="field" style="margin:0"><span>${escHtml(s.label)}</span><input class="form-input bm-site" data-site="${escAttr(s.key)}" type="number" inputmode="decimal" min="1" max="100" step="0.1" placeholder="in" style="width:100%"></label>`)
            .join("");
