...
- The frontend source of truth now lives under `src/client/**/*.ts`, emitted through `scripts/build-client.mjs` into stable `public/js` filenames. `public/js/10-boot.js` is the only handwritten app-shell shim; live route-console smoke is green on the preserved `/app/*` paths, and `npm run smoke:browser` now provides an opt-in Chrome/CDP route/global smoke with preserved path-URL assertions, section-state assertions, Today add/off-plan exercise through real set logging, and Chat attachment focus recovery. Remaining frontend work is shrinking large typed screen/controller modules, retiring broad globals/state where practical, and expanding browser workflow smoke beyond Today/Chat.
- `03-today.js` is now generated from `src/client/today-screen.ts` with a strict client TypeScript check, locally scoped output, and an explicit compatibility bridge for only `renderToday`, `reshapeToday`, `postExerciseMode`, `reconnectSessionSuggest`, and `reconnectDayReadOverride`. Today add/off-plan exercise form wiring now lives in `src/client/today-add-exercise-controller.ts`, preserving skipped-card restoration, typed-card protection, last-set prefill, and pending off-plan state while the screen passes explicit dependencies. Today session-suggestion markup lives in `src/client/today-session-suggest-client.ts`; request lifecycle, reconnect, and log-these handoff now live in `src/client/today-session-suggest-controller.ts`; Today session/logging behavior now lives in `src/client/today-session-controller.ts`, covering finish/reopen/history, feedback save/edit, set logging/deletes, skip/undo, off-plan removal, rest start, and progression invalidation while the screen keeps only compatibility wrappers. `09-plan-chat.js` is generated from `src/client/chat-screen.ts`; Plan editor and Plan Endurance orchestration live in typed sources. `02-ui.js` is generated from `src/client/ui-shell.ts`, with generic full-screen detail overlay mechanics delegated to `src/client/detail-overlay-client.ts` and shared collapse/expand delegated to `src/client/ui-motion-client.ts`; `04-capture.js` is generated from `src/client/capture.ts`; `05-progress.js` is generated from `src/client/progress-screen.ts`; `06-coach-meals.js` is generated from `src/client/coach-meals-screen.ts`, with Day Fuel load/edit delegated to `src/client/day-fuel-controller.ts` and Meal Recipe sheet/job lifecycle delegated to `src/client/meal-recipe-controller.ts`; `07-me-health.js` is generated from `src/client/me-health-screen.ts`; `08-me-records.js` is generated from `src/client/me-records-screen.ts`; Settings screen rendering is generated from `src/client/settings-screen.ts`, with Settings Agents markup delegated to `src/client/settings-agents-client.ts`. Health directive markup and execution are split: `health-directives-client.ts` owns pure rendering, while `health-directives-loader-client.ts` owns `/directives`, `/evidence/summary`, evidence toggles, done/dismiss, and derive-refresh wiring so direct `/app/me/health/read` routes do not depend on screen-local functions from another bundle. Do not edit generated `public/js` files directly; run `npm run client:check`, `npm run client:build`, `node scripts/check-client-build-output.mjs`, `npm run public:check`, and `node scripts/check-sw-cache.mjs` after browser changes; run `npm run smoke:browser` before release when Chrome is available.
...
Latest UI shell migration: `src/client/ui-shell.ts` now owns the served `/js/02-ui.js` file, preserving header date controls, toast/delete pattern, segmented nav, transitions, skeleton/loading states, progressive art, detail overlays, enrichment polling, food/exercise detail bridges, script order, and PWA cache alignment behind strict client TypeScript; the agent-login terminal lives in `src/client/agent-login-client.ts`.
...
- [x] Move the shared UI shell (`02-ui.js`) to `src/client/ui-shell.ts` while preserving mutable globals, segment registries, detail overlays, progressive art, enrichment polling, script order, and PWA cache alignment. The agent-login modal is now owned by `src/client/agent-login-client.ts`.
...
Latest Settings controller reduction: `src/client/settings-agents-client.ts` now owns the Settings Agents slice and agent-card markup; `src/client/settings-screen.ts` keeps working-model mutation, save-bar behavior, route pruning, lazy agent info/model fetches, CLI update polling, and login handoff.
...
- [ ] Shrink the large typed screen/controllers (`today-screen`, `chat-screen`, `progress-screen`, `coach-meals-screen`, `me-health-screen`, `me-records-screen`, `settings-screen`, `ui-shell`, and `capture`) into smaller responsibility modules.
...
- [ ] Expand the automated browser workflow lane for deeper Today skip/cardio, Chat send/stream/reconnect, Health marker/records, Settings Data, Plan, and Progress interactions.
