...
- [x] Cover mobile Chat keyboard/input contracts in focused tests while browser smoke remains the final device check. The file-picker path now has contracts around `resetChatFocusAfterNativePicker`, `cairn:keyboard-settle`, and the mobile viewport chat-focus grace window.
