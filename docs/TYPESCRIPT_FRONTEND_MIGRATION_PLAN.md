...
- Mobile Chat input and keyboard behavior is now guarded by the typed `src/client/app/mobile-viewport.ts` slice plus focused `src/client/chat-attachment-client.ts` and `09-plan-chat.js` event handling: geometry, not stale focus, drives keyboard state; stale iOS textarea focus is released after keyboard dismissal; native file-picker return remeasures through keyboard settling; send-button touch input stays focused and sends on pointer-up; touch Enter remains a newline; and `test/chatKeyboardUi.test.js` / `test/chatPhotoUi.test.js` / `test/clientMobileViewport.test.js` cover the contract.
