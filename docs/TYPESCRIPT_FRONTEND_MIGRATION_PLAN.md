...
- [ ] Shrink the large typed screen controllers (`today-screen`, `chat-screen`, `progress-screen`, `coach-meals-screen`, `me-health-screen`, `me-records-screen`, and `settings-screen`) into smaller responsibility modules.
- [x] Run a live route/cache browser smoke for preserved paths (`/`, `/app/today`, `/app/plan/meals`, `/app/progress/energy`, `/app/me/standing`, `/app/me/health/read`, `/app/me/health/records`, `/app/chat`, `/app/settings/data`) with console-error capture.
- [x] Add an opt-in automated browser route/global smoke lane (`npm run smoke:browser`) that boots the built server, launches Chrome/CDP with a fresh profile, loads preserved `/app/*` paths, fails on runtime/console/script/API load errors, and asserts critical app globals.
- [ ] Add a dedicated automated browser workflow lane for critical Today, Chat, Health, Settings, Plan, and Progress interactions.
