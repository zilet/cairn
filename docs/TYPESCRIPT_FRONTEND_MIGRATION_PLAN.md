...
- Add `src/client/` with one tiny generated output slice first.
- Add `tsconfig.client.build.json`.
- Add `npm run client:build` and `npm run client:check`.
- Decide and document whether generated JS is committed during the transition.
- Add a generated-output freshness check if generated JS remains committed.
- Update Docker build once `public/js` is generated by `npm run build`.
