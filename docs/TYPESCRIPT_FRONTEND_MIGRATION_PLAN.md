...
- `src/contracts/client.ts`, `src/contracts/client-routes.ts`, `src/contracts/client-api.ts`, `src/contracts/client-api-coverage.ts`, and `src/contracts/client-compat.ts` establish shared backend-to-client route/payload contracts, API path coverage, owned broad-response waivers, and compile-time backend assignability checks. The current client path registry has no temporary unknown waivers; deterministic training-brain reads for run zones, weekly run plan, muscle trajectory, test-week cadence, DEXA targeting, program balance, program adjustments, program block lifecycle responses, offline guideline references, and injury-impact reads are now narrow DTOs with backend assignability checks. Remaining contract work is burning down the explicitly owned broad DTO envelopes.
...
- The PWA API seam now has a shared coverage registry: source and generated client `api()`, `cachedApi()`, `enqueueJob()`, SWR `path`, and durable-job `path` literals must match a typed contract path or a named temporary waiver. There are currently no unknown waivers; any exact response that still uses `ClientJsonObject` or `ClientJsonArray` must have an owner/reason entry in `CLIENT_API_BROAD_RESPONSE_WAIVERS` until the owning screen narrows its payload model.
...
Status: in progress. The contract slice added `src/contracts/client-api.ts` for shared PWA response DTOs, `src/contracts/client-api-coverage.ts` for every current client API/SWR/job path, backend assignability checks for Settings/Profile/Goal/Plan/Exercise/Health-document payloads, and a typed `listExercises()` row so SQLite row leakage is not the public contract. The coverage gate now scans `src/client/**/*.ts` and generated `public/js` for direct API calls, `cachedApi`, `enqueueJob`, SWR path literals, and durable job path literals; the current unknown-waiver list is empty, and broad exact response DTOs require owned waivers. The deterministic training-brain endpoints `/run-zones`, `/run-plan`, `/muscle-trajectory`, `/test-week`, `/dexa-targeting`, `/program/balance`, `/program/adjustments`, `/program/blocks`, `/program/blocks/active`, `/guidelines`, and `/injury-impacts` now use narrow client DTOs with backend assignability checks.
