...
- Latest modularity waves add typed ownership for progressive art (`art-controller`), shared UI feedback/loading (`ui-feedback-client`), Today side loaders (`today-side-loaders`), Today Brief orchestration (`today-brief-controller`), Capture weekly/insight reads (`capture-reads-client`), Health Read/Standing/Markers orchestration (`health-read-controller`, `health-standing-controller`, `health-markers-controller`), Health document upload (`health-doc-upload-controller`), Chat message rendering, Chat header orchestration, and composer-focus recovery (`chat-message-client`, `chat-header-controller`, `chat-composer-focus-client`), Meals planner and swap flows (`meal-planner-controller`, `meal-swap-controller`), Plan editor orchestration (`plan-editor-controller`), Progress Program orchestration (`progress-program-controller`), Settings Agents orchestration (`settings-agents-controller`), Capture provenance (`capture-provenance-client`), Me memory (`me-memory-controller`), and Me Life orchestration (`life-controller`).
...
Latest reduction wave: Plan editor orchestration now lives in `src/client/plan-editor-controller.ts`, Chat header history/session controls live in `src/client/chat-header-controller.ts`, and Me Life timeline orchestration lives in `src/client/life-controller.ts`. Browser workflow smoke now also verifies planned-cardio skip/restore without losing `/app/today`.

Updated integration note: `03-today.js`, `02-ui.js`, `04-capture.js`, `07-me-health.js`, `08-me-records.js`, and `09-plan-chat.js` are still stable public compatibility filenames, but more runtime responsibility now lives in smaller generated modules loaded before those consumers. `public/js/10-boot.js` remains the tiny final startup shim; every other served client script should stay build-owned. Current top source-size targets are `today-screen.ts` (1215 lines), `me-health-screen.ts` (540), `today-session-controller.ts` (486), `ui-shell.ts` (447), `today-brief-controller.ts` (447), `chat-turn-client.ts` (435), `settings-screen.ts` (429), `health-client.ts` (427), `health-read-controller.ts` (417), `chat-screen.ts` (412), `progress-chart-client.ts` (387), `progress-screen.ts` (384), `meal-swap-controller.ts` (372), `me-records-screen.ts` (366), `me-profile-controller.ts` (360), `health-standing-client.ts` (348), `plan-editor-controller.ts` (347), `capture.ts` (345), `coach-meals-screen.ts` (335), `plan-endurance-client.ts` (334), `me-records-health-doc-controller.ts` (332), `meal-plan-client.ts` (326), `agent-login-client.ts` (313), and `capture-reads-client.ts` (303). Future work should keep extracting cohesive controller/render/data modules from those files, not edit generated public output directly.
