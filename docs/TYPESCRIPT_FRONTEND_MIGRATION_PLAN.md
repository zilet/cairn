...
- [ ] Shrink the remaining large typed screen/controllers (`today-screen`, `me-health-screen`, `me-health-dependencies`, `coach-meals-screen`, `ui-shell`, `progress-screen`, `meal-swap-controller`, `chat-turn-client`, `agent-login-client`, `me-records-screen`, `capture-reads-client`, `meal-planner-controller`, `today-session-controller`, `today-plan-session-preparation`, `agent-job-client`, `progress-history-client`, `today-rail-controller`, `progress-chart-client`, `capture`, `exercise-detail-controller`, `chat-screen`, `life-controller`, `meal-recipe-controller`, `health-read-client`, `health-doc-actions-controller`, `today-dependencies`, `settings-screen`, `plan-editor-controller`, `health-markers-client`, and `health-read-controller`) into smaller responsibility modules. Latest slices moved Me Health inner-tab routing to `me-health-tabs-controller`, Me Profile form rendering/normalization to `me-profile-form-client`, Chat turn records, layout, and composer wiring to `chat-turn-records-client` / `chat-layout-client` / `chat-composer-controller`, Progress data normalization, line-chart geometry, chart scrub wiring, route dependencies, and History card model derivation to `progress-data-client` / `progress-line-chart-model` / `progress-chart-scrub-client` / `progress-route-deps-client` / `progress-history-client`, Meal row/day rendering and Meal Swap normalization to `meal-row-client` / `meal-swap-data-client`, Capture voice to `capture-voice-client`, Today data loading, dependency assembly, main shell, plan-surface rendering, render-state derivation, and Brief DOM actions to `today-data-loader` / `today-dependencies` / `today-main-shell-client` / `today-plan-surface-renderer` / `today-render-state-client` / `today-brief-actions-client`, Plan Endurance model/render helpers to `plan-endurance-model`, Health marker ordering to `health-marker-order-client`, Health Standing primitives to `health-standing-primitives-client`, Records document actions and Health Share orchestration to `health-doc-actions-controller` / `health-share-controller`, Plan editor form handling to `plan-editor-form-client`, app state shape to `src/contracts/client-state.ts`, UI header/date controls to `ui-header-client`, and UI transition behavior to `ui-view-transitions-client`.
