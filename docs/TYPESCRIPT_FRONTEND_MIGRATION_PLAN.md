...
Latest runtime/composition split: `src/client/today-screen-runtime.ts` owns Today's API/SWR/dependency/legacy-wrapper adapters, dropping `today-screen.ts` to 390 lines focused on render orchestration; `src/client/me-health-screen-composition.ts` owns Me/Health section routing and controller delegation, dropping `me-health-screen.ts` to 325 lines as a compatibility facade; `src/client/chat-turn-monitor-client.ts` owns Chat turn EventSource lifecycle and reconnect advancement, dropping `chat-turn-client.ts` to 258 lines; `src/client/agent-job-records-client.ts` owns durable job record/SSE parsing, dropping `agent-job-client.ts` to 251 lines. The public shell now has 184 generated browser outputs, 186 boot scripts, 190 core service-worker assets, and `public/sw.js` cache `cairn-v385`.
...
- [ ] Shrink the remaining large typed screen/controllers (`today-screen-runtime`, `today-screen`, `me-health-screen-composition`, `me-health-screen`, `agent-login-client`, `me-records-screen`, `capture-reads-client`, `meal-planner-controller`, `today-session-controller`, `today-plan-session-preparation`, `ui-shell`, `progress-history-client`, `today-rail-controller`, `progress-chart-client`, `capture`, `exercise-detail-controller`, `chat-screen`, `life-controller`, `meal-recipe-controller`, `health-read-client`, `health-doc-actions-controller`, `today-dependencies`, `plan-editor-controller`, `health-markers-client`, `progress-screen`, `health-read-controller`, `agent-job-client`, and `chat-turn-client`) into smaller responsibility modules. Latest slices moved Me Health inner-tab routing to `me-health-tabs-controller`, Me Health composition/controller delegation to `me-health-screen-composition`, Me Health dependency factories to `me-health-controller-deps`, Me Profile form rendering/normalization to `me-profile-form-client`, Chat turn records, EventSource monitoring, layout, and composer wiring to `chat-turn-records-client` / `chat-turn-monitor-client` / `chat-layout-client` / `chat-composer-controller`, durable job record parsing to `agent-job-records-client`, Progress data normalization, line-chart geometry, chart scrub wiring, route dependencies, History card model derivation, and trend/weight rendering to `progress-data-client` / `progress-line-chart-model` / `progress-chart-scrub-client` / `progress-route-deps-client` / `progress-history-client` / `progress-trend-weight-client`, Meal row/day rendering, Meal Swap normalization, and Meal Swap row actions to `meal-row-client` / `meal-swap-data-client` / `meal-swap-row-actions-controller`, Capture voice to `capture-voice-client`, Today data loading, dependency assembly, main shell, plan-surface rendering, render-state derivation, compatibility bridges, runtime adapters, and Brief DOM actions to `today-data-loader` / `today-dependencies` / `today-main-shell-client` / `today-plan-surface-renderer` / `today-render-state-client` / `today-compatibility-bridges` / `today-screen-runtime` / `today-brief-actions-client`, Plan Endurance model/render helpers to `plan-endurance-model`, Coach proposal wiring to `coach-proposal-controller`, Health marker ordering to `health-marker-order-client`, Health Standing primitives to `health-standing-primitives-client`, Records document actions and Health Share orchestration to `health-doc-actions-controller` / `health-share-controller`, Settings Sources/Automation orchestration to `settings-sources-automation-controller`, Plan editor form handling to `plan-editor-form-client`, app state shape to `src/contracts/client-state.ts`, UI header/date controls to `ui-header-client`, UI segment/discipline behavior to `ui-segments-client`, and UI transition behavior to `ui-view-transitions-client`.
