...
- Latest checkpoint: Today plan/session async data loading now has its own typed helper (`today-plan-session-data-client`) for last-set cache reads, adaptive prescription loading, and cardio context loading. The existing preparer remains the stable compatibility API for selection/matching/prefill assembly.
