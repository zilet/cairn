...
- Critical extracted helpers are now TypeScript sources: app core state, app route bridge, app route-sync wrapper, app render dispatcher, app tab controller, app onboarding, app startup, app job reconnectors, app mobile viewport guards, app service-worker lifecycle, app discipline primer, API, SWR, routes, date/html/format helpers, Today agenda/training helpers, Settings helpers, Chat helpers, and Health helpers.
