...
- Latest modularity waves add typed ownership for progressive art (`art-controller`), shared UI feedback/loading (`ui-feedback-client`), shared transient actions (`ui-actions-client`), shared view-transition behavior (`ui-view-transitions-client`), Today side loaders (`today-side-loaders`), Today plan/session preparation (`today-plan-session-preparation`), Today primary data/SWR loading (`today-data-loader`), Today post-render DOM wiring (`today-post-render-wiring`), Today Brief orchestration and override job handling (`today-brief-controller`, `today-brief-override-client`), Today weekly compass/pace rendering (`today-compass-client`), Today session feedback and skip/off-plan wiring (`today-session-feedback-client`, `today-session-skip-client`), Capture weekly/insight reads (`capture-reads-client`), Health Read/Standing/Markers orchestration (`health-read-controller`, `health-standing-controller`, `health-markers-controller`), Health Read synthesis job/rendering and supplement input/delete wiring (`health-read-synthesis-client`, `health-read-supplements-client`), Health evidence/directive rendering (`health-evidence-client`), Health document upload (`health-doc-upload-controller`), Me Health log rendering (`me-health-log-renderer`), Me Health inner-tab routing (`me-health-tabs-controller`), Chat message rendering, Chat turn record parsing/draft storage, Chat stream markdown state, Chat header orchestration, composer-focus recovery, and composer wiring (`chat-message-client`, `chat-turn-records-client`, `chat-turn-stream-state-client`, `chat-header-controller`, `chat-composer-focus-client`, `chat-composer-controller`), Meals planner and swap flows plus swap data normalization (`meal-planner-controller`, `meal-swap-controller`, `meal-swap-data-client`), Plan editor orchestration (`plan-editor-controller`), Progress line-chart geometry (`progress-line-chart-model`), Progress route dependency adapters (`progress-route-deps-client`), Progress Program orchestration (`progress-program-controller`), Settings surface data/render helpers and Agents orchestration (`settings-surface-client`, `settings-agents-controller`), Capture provenance (`capture-provenance-client`), Me memory (`me-memory-controller`), and Me Life orchestration (`life-controller`).
...
- [ ] Shrink the remaining large typed screen/controllers (`today-screen`, `me-health-screen`, `me-records-screen`, `today-brief-controller`, `me-profile-controller`, `chat-turn-client`, `progress-screen`, `health-standing-client`, `plan-editor-controller`, `capture`, `ui-shell`, `coach-meals-screen`, `plan-endurance-client`, `me-records-health-doc-controller`, `health-client`, `progress-chart-client`, `meal-plan-client`, `meal-swap-controller`, `agent-login-client`, `capture-reads-client`, `meal-planner-controller`, `today-session-controller`, `today-plan-session-preparation`, `agent-job-client`, `today-rail-controller`, `exercise-detail-controller`, `chat-screen`, `life-controller`, `meal-recipe-controller`, `health-read-client`, `settings-screen`, `health-markers-client`, `progress-history-client`, and `health-read-controller`) into smaller responsibility modules. Latest slices moved Me Health inner-tab routing to `me-health-tabs-controller`, Chat turn records and composer wiring to `chat-turn-records-client` / `chat-composer-controller`, Progress line-chart geometry and route dependencies to `progress-line-chart-model` / `progress-route-deps-client`, Meal Swap normalization to `meal-swap-data-client`, Today data loading to `today-data-loader`, and UI transition behavior to `ui-view-transitions-client`.
