...
Latest Me records screen migration: `src/client/me-records-health-doc-controller.ts` now owns Records list loading, upload handoff, and deep-link scrolling, while `src/client/health-doc-actions-controller.ts` owns per-document row behavior: date edit/save/cancel, re-analysis polling, collapse toggles, delete, picture-cache decrement, marker refreshes, and row rewiring. `src/client/me-records-screen.ts` remains the served `/js/08-me-records.js` screen file for Health Markers/Records/Share/Learned tab bridges, Life timeline add/edit/delete, and Family roster add/edit/delete wiring behind strict client TypeScript.
...
- [ ] Shrink the remaining large typed screen/controllers (`today-screen`, `me-health-screen`, `me-records-screen`, `today-brief-controller`, `ui-shell`, `coach-meals-screen`, `plan-endurance-client`, `health-client`, `progress-chart-client`, `meal-plan-client`, `progress-screen`, `meal-swap-controller`, `chat-turn-client`, `agent-login-client`, `capture-reads-client`, `meal-planner-controller`, `today-session-controller`, `today-plan-session-preparation`, `agent-job-client`, `today-rail-controller`, `capture`, `exercise-detail-controller`, `chat-screen`, `life-controller`, `meal-recipe-controller`, `health-read-client`, `health-doc-actions-controller`, `settings-screen`, `health-markers-client`, and `health-read-controller`) into smaller responsibility modules. Latest slices moved Me Health inner-tab routing to `me-health-tabs-controller`, Me Profile form rendering/normalization to `me-profile-form-client`, Chat turn records, layout, and composer wiring to `chat-turn-records-client` / `chat-layout-client` / `chat-composer-controller`, Progress data normalization, line-chart geometry, route dependencies, and History card model derivation to `progress-data-client` / `progress-line-chart-model` / `progress-route-deps-client` / `progress-history-client`, Meal Swap normalization to `meal-swap-data-client`, Capture voice to `capture-voice-client`, Today data loading, main shell, plan-surface rendering, and render-state derivation to `today-data-loader` / `today-main-shell-client` / `today-plan-surface-renderer` / `today-render-state-client`, Health Standing primitives to `health-standing-primitives-client`, Records document actions to `health-doc-actions-controller`, Plan editor form handling to `plan-editor-form-client`, app state shape to `src/contracts/client-state.ts`, and UI transition behavior to `ui-view-transitions-client`.
