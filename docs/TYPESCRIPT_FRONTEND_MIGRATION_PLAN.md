...
- Latest modularity waves add typed ownership for progressive art (`art-controller`), shared UI feedback/loading (`ui-feedback-client`), Today side loaders (`today-side-loaders`), Today Brief orchestration (`today-brief-controller`), Capture weekly/insight reads (`capture-reads-client`), Health Read/Standing/Markers orchestration (`health-read-controller`, `health-standing-controller`, `health-markers-controller`), Health document upload (`health-doc-upload-controller`), Chat message rendering and composer-focus recovery (`chat-message-client`, `chat-composer-focus-client`), Meals planner and swap flows (`meal-planner-controller`, `meal-swap-controller`), Progress Program orchestration (`progress-program-controller`), Settings Agents orchestration (`settings-agents-controller`), Capture provenance (`capture-provenance-client`), and Me memory (`me-memory-controller`).
...
- [ ] Shrink the remaining large typed screen/controllers (`today-screen`, `me-records-screen`, `me-health-screen`, `chat-screen`, `plan-editor-client`, `today-session-controller`, `ui-shell`, `today-brief-controller`, `chat-turn-client`, `settings-screen`, `health-client`, `health-read-controller`, `progress-chart-client`, `progress-screen`, and `meal-swap-controller`) into smaller responsibility modules.
