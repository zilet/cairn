...
- [ ] Shrink the remaining large typed screen/controllers (`today-screen`, `me-health-screen`, `me-records-screen`, `today-brief-controller`, `health-standing-client`, `capture`, `ui-shell`, `coach-meals-screen`, `plan-endurance-client`, `me-records-health-doc-controller`, `health-client`, `progress-chart-client`, `meal-plan-client`, `progress-screen`, `meal-swap-controller`, `chat-turn-client`, `agent-login-client`, `capture-reads-client`, `meal-planner-controller`, `today-session-controller`, `today-plan-session-preparation`, `agent-job-client`, `today-rail-controller`, `exercise-detail-controller`, `chat-screen`, `life-controller`, `meal-recipe-controller`, `health-read-client`, `settings-screen`, `health-markers-client`, `progress-history-client`, and `health-read-controller`) into smaller responsibility modules. Latest slices moved Me Health inner-tab routing to `me-health-tabs-controller`, Me Profile form rendering/normalization to `me-profile-form-client`, Chat turn records, layout, and composer wiring to `chat-turn-records-client` / `chat-layout-client` / `chat-composer-controller`, Progress data normalization, line-chart geometry, and route dependencies to `progress-data-client` / `progress-line-chart-model` / `progress-route-deps-client`, Meal Swap normalization to `meal-swap-data-client`, Today data loading and plan-surface rendering to `today-data-loader` / `today-plan-surface-renderer`, Plan editor form handling to `plan-editor-form-client`, and UI transition behavior to `ui-view-transitions-client`.
