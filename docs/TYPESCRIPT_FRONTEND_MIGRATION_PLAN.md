...
Status: in progress. The Today team's capture/provenance/insight script now has TypeScript source in `src/client/capture.ts`, preserving the served `/js/04-capture.js` filename and its classic-script globals for `03-today.js`, meals, and job reconnectors. Shared activity-row rendering now lives in `src/client/today-activity-client.ts`, reducing `04-capture.js` and Health activity history dependence on `03-today.js`. The Today Brief and focus-bar renderer now lives in `src/client/today-brief-client.ts`, reducing the remaining `03-today.js` day-read surface while preserving fetch/cache/job wiring. The Today session-suggestion renderer now lives in `src/client/today-session-suggest-client.ts`, reducing the remaining `03-today.js` agentic suggestion surface while preserving runOp/reconnect/log-these behavior. The Progress program summary and program-block helpers now live in `src/client/progress-program-summary-client.ts` and `src/client/progress-program-block-client.ts`, reducing the remaining `05-progress.js` Program surface.
