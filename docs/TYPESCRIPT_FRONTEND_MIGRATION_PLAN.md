...
- Latest modularity waves add typed ownership for progressive art (`art-controller`), shared UI feedback/loading (`ui-feedback-client`), Today side loaders (`today-side-loaders`), Today Brief orchestration (`today-brief-controller`), Today weekly compass/pace rendering (`today-compass-client`), Capture weekly/insight reads (`capture-reads-client`), Health Read/Standing/Markers orchestration (`health-read-controller`, `health-standing-controller`, `health-markers-controller`), Health evidence/directive rendering (`health-evidence-client`), Health document upload (`health-doc-upload-controller`), Chat message rendering, Chat header orchestration, and composer-focus recovery (`chat-message-client`, `chat-header-controller`, `chat-composer-focus-client`), Meals planner and swap flows (`meal-planner-controller`, `meal-swap-controller`), Plan editor orchestration (`plan-editor-controller`), Progress Program orchestration (`progress-program-controller`), Settings surface data/render helpers and Agents orchestration (`settings-surface-client`, `settings-agents-controller`), Capture provenance (`capture-provenance-client`), Me memory (`me-memory-controller`), and Me Life orchestration (`life-controller`).
...
Latest reduction waves: Plan editor orchestration now lives in `src/client/plan-editor-controller.ts`, Chat header history/session controls live in `src/client/chat-header-controller.ts`, and Me Life timeline orchestration lives in `src/client/life-controller.ts`. The newest split moves Health evidence/directive rendering into `src/client/health-evidence-client.ts`, Settings data/status/source/automation render helpers into `src/client/settings-surface-client.ts`, and Today's weekly compass/pace-offer rendering into `src/client/today-compass-client.ts`. Browser workflow smoke also verifies planned-cardio skip/restore without losing `/app/today`.

Updated integration note: `03-today.js`, `02-ui.js`, `04-capture.js`, `07-me-health.js`, `08-me-records.js`, and `09-plan-chat.js` are still stable public compatibility filenames, but more runtime responsibility now lives in smaller generated modules loaded before those consumers. `public/js/10-boot.js` remains the tiny final startup shim; every other served client script should stay build-owned. Current top source-size targets are `today-screen.ts` (1130 lines), `me-health-screen.ts` (540), `today-session-controller.ts` (486), `ui-shell.ts` (447), `today-brief-controller.ts` (447), `chat-turn-client.ts` (435), `health-read-controller.ts` (417), `chat-screen.ts` (412), `progress-chart-client.ts` (387), `progress-screen.ts` (384), `meal-swap-controller.ts` (372), `me-records-screen.ts` (366), `me-profile-controller.ts` (360), `health-standing-client.ts` (348), `plan-editor-controller.ts` (347), `capture.ts` (345), `coach-meals-screen.ts` (335), `plan-endurance-client.ts` (334), `me-records-health-doc-controller.ts` (332), `health-client.ts` (331), `meal-plan-client.ts` (326), `agent-login-client.ts` (313), `capture-reads-client.ts` (303), and `meal-planner-controller.ts` (300). Future work should keep extracting cohesive controller/render/data modules from those files, not edit generated public output directly.
...
- [ ] Shrink the remaining large typed screen/controllers (`today-screen`, `me-health-screen`, `today-session-controller`, `ui-shell`, `today-brief-controller`, `chat-turn-client`, `health-read-controller`, `chat-screen`, `progress-chart-client`, `progress-screen`, `meal-swap-controller`, `me-records-screen`, `me-profile-controller`, `health-standing-client`, `plan-editor-controller`, `capture`, `coach-meals-screen`, `plan-endurance-client`, `me-records-health-doc-controller`, `health-client`, `meal-plan-client`, `agent-login-client`, `capture-reads-client`, and `meal-planner-controller`) into smaller responsibility modules.
...
- [ ] Expand the automated browser workflow lane for Chat send/stream/reconnect interactions and any synced-cardio edge cases that need real browser coverage.
