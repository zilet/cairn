...
- Docker now runs the client build in the builder stage and overlays generated `public/js` into the runtime image from that builder, so source-build and Pi Docker deploys build browser output from TypeScript instead of depending on checked-in compiled JS.
...
- `npm run verify`: runs client typecheck/buildability, public script/cache checks, backend build, launch checks, and tests.
- Source-control decision: generated `public/js` output is ignored by git except for `public/js/10-boot.js`; local dev, Docker, and Pi source runs build it from `src/client`.

Current Docker contract for generated frontend output:

- builder stage copies package manifests, server/client tsconfigs, `scripts/build-client.mjs`, `src/`, and `public/js/10-boot.js`.
...
- source-build docs and Pi deploy stay aligned with this path; `.dockerignore` excludes generated `public/js/*.js` from the build context so stale local output cannot leak into the image.
...
- Cache Agent: keep `public/index.html`, generated client build output, git-ignore hygiene, and `public/sw.js` aligned.
...
- [x] Expand the automated browser workflow lane for Family CRUD, Plan editor save + Meals recipe, Health Records date-edit, and Settings Agents/Sources/Automation. Current coverage now also includes: adding/editing/deleting a family member through the real `/app/me/family` form with API round-trip checks; editing a Plan editor target weight through the save bar and confirming it via the plan API, then opening/closing the cached Monday-dinner recipe sheet on `/app/plan/meals`; expanding/collapsing and date-editing a `/app/me/health/records` document through its real controls with an API round-trip check; and, on Settings, checking the smoke agent's card/detail disclosure, flipping the weekly auto-coach toggle, and triggering a real credential-less Garmin sync failure across the Agents/Sources/Automation sub-tabs, both confirmed via the settings API.
...
| --- | --- | --- |
| Docker serves stale generated JS | Runtime images must not depend on local or checked-in compiled `public/js` | Docker builder runs `npm run build`, `.dockerignore` excludes generated `public/js/*.js`, and runtime overlays `/app/public/js` from the builder; keep deploy checks and Docker copy path aligned |
