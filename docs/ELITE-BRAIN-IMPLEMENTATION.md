...
- **Heads-up autonomy under `lead` only (2026-08-17 ruling).** Two cases that used to stop and ask now announce with the server-owned one-tap Undo: a REVERSIBLE high-risk change, and a goal change (`goal_change`, or the `goal_identity` flag). What did not move: clinical stays `clinician`, an irreversible action stays `ask`, a `user_locked` decision stays `ask`, and a refused safety clamp stays `ask`. `announce_first` and `review_everything` return exactly their previous answers in every case. Pinned by `test/headsUpAutonomy.test.js`.
- A goal date can now adapt from the signals: `applyGoalDateAdaptationWithAutonomy()` (`src/domain/brain/autonomy-service.ts`) owns the policy and the ledger, never the arithmetic — the caller says the date should move and to where. It refuses an adaptation whose `from` no longer matches the live profile (stale arithmetic is absent, not approximate), writes nothing itself, and lands the date at its natural boundary through `applyDueAnnouncedDecisions`, which is also where the `goal_date` rollback snapshot is taken. The deterministic caller is the cut-target derivation, offered on the nutrition check-in cadence.
- Quiet nutrition changes wait for the next day; an in-progress training session is never rewritten underneath the athlete.
- Announced changes land at their effective boundary unless canceled. 90-day veto-rate demotion still limits overreach; the surprise budget is three material changes per domain-week (`SURPRISE_BUDGET_PER_DOMAIN_WEEK`), raised from one because a coach that noticed something on Tuesday had to sit on it until Monday. The athlete's OWN asks (`explicit_user_request`, `athlete_requested_restructure`) never count against it — two Sunday restructure requests used to defer that week's evolution. **A budget miss is a delay, never an ask**: `applyDueAnnouncedDecisions` pushes the decision out a day and leaves it announced/pending (its third return field, `delayed`), and termination stays with the ceilings that own it — the proposal age gate and evidence freshness. A safety response is never rationed.
- `thawParkedReviewDecisions()` re-offers decisions frozen at `status:'review'` through TODAY's policy — deterministic, agent-free, and skipped entirely under `review_everything`. It never touches one parked behind a floor (`THAW_FLOOR_REASON_CODES`: clinical, clinical ceiling, safety floor, user lock), and it stamps `thaw_attempted` into the decision context so each one is re-offered once rather than every sweep. A held draft whose evidence has moved is superseded with a receipt, never adopted.
- **Only floors wait on the athlete (2026-09-25 ruling).** A case conference is routed action by action: the clinician floor is judged on its executable revision alone, a clinical parallel action becomes a "for you and your doctor" note, and reversibility is the server's fact. Under `lead` a model-requested ask and an unresolved coaching trade-off announce (apply-and-tell with Undo); unresolved safety conflicts hold as `safety_floor`; advice never parks. The thaw re-files parked advisories, re-reads conference-marked clinician holds once by today's rule, and closes the conference door under lead. An athlete's own request applies (no veto-rate demotion, boundary drift tolerated) or is answered in chat when it cannot land. Details: `docs/ARCHITECTURE.md` "Only floors wait on the athlete". Pinned by `test/caseConference.test.js`, `test/headsUpAutonomy.test.js`, `test/boundaryOutcomeTaxonomy.test.js`, `test/athleteRequestOutcome.test.js`.
...
- **A resolved conflict must CITE (2026-08-24 ruling).** Naming a conflict in `resolved_conflicts` is no longer the test. A resolution counts only when its `evidence_key` is genuinely one of the `evidence_keys` of a specialist whose domain is a PARTY to that conflict, and its one-line resolution says how that evidence settles it — the old contract asked the conductor to list every conflict, so an echo of the server's own list dissolved every conflict the server had just detected. An uncited claim leaves the conflict unresolved and the existing demotion applies untouched, and the conductor prompt now says so plainly (leave a conflict you cannot honestly close out of the array). `clinical_autonomy` has NO parties by construction: the clinician floor is the server's, no opinion may attest it away, and no conductor can close it. That also makes it terminal rather than a failure — a conference held at the clinical floor is finished work, so it stamps its month and stops instead of burning the month's retries to arrive at the same clinician-directed decision already waiting for the human. Detection itself is typed predicates over context values (`src/domain/brain/conference-conflicts.ts`), read from the UNTRUNCATED context, under the law that absent evidence never fires a conflict.
- **The clinical floor is RELEVANCE, not co-occurrence (2026-09-22 ruling).** `clinical_autonomy` used to fire on ANY act-now health focus plus ANY training/nutrition directive (the `uncertain` HRV row included), and a conference that produced any revision at all became its own lever — so a lipid finding clinician-gated a squat-load hold, and every whole-person conference since August ended parked. Now an act-now finding's LEVERS are only its settled (non-`uncertain`, non-`watch`) directives of an act-now marker group, each carrying the areas its own words name (endurance/strength in training; energy/protein/fat/carbohydrate in nutrition; none named = the whole domain). The pre-revision rule is the specialists' heads-up; what the server ENFORCES is `clinicalAutonomyFromRevision(inputs, revision)`: the revision's domain and touched areas must meet a lever, or the revision must name an act-now marker. Advice with no revision is never gated here. What a change SAYS — a diagnosis, a medication, a dose — still holds the floor through `clinicalActionText`, so a genuinely clinical change always asks, and the floor stays deterministic in both directions.
- **The team (2026-09-22).** Specialists carry CHARTERS (`src/domain/brain/specialist-charters.ts`): strength coach (`training`), endurance coach (`endurance` — a conference seat whose decisions file under `training`), dietitian, physio (`recovery`), physician-informational (`health`), lifestyle. Each names the ONE next step toward its milestone in `road_ahead` and the trade-off it asks of the others; a hold must say what earns the next step. The conductor reconciles in the block's priority order (`blockPriority`, `src/repo/road-ahead.ts` — the active block's focus leads, else stated intent; the cut ranks after the training goals it serves unless leanness is the athlete's first priority), and the advice-only fallback follows the lead goal's seat instead of defaulting to recovery. The agents' snapshot is the `case_conference` promptData site with `road_ahead` first, so the 50-key bound no longer silently drops the health and signal reads.
- A monthly, phase-boundary, or unexplained-regression scheduler check queues the standing whole-person revision conference. **The weekly team review** (`enqueueTeamReviewIfDue`, `src/scheduler.ts`) runs on the coach slot (Sunday evening by default) with five seats at one call each plus the conductor (~6 spawns), owns that slot's weekly plan evolution, and lands through the same conference → proposal → autonomy path — pull-only, a structural change at Monday's boundary. It only runs in its own evening or the morning after, and a busy spawn permit defers the durable job rather than failing it.
