...
| Wave 5 - High-risk frontend migration | Mostly complete | generated browser source now lives under `src/client`; live route-console smoke is green; Today session-suggest, session logging, and mobile chat picker-focus are now guarded; large screen-controller modules still need shrinking |
