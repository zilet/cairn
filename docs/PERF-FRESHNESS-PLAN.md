...
- **E. Stream prose into cards** (`perf2/track-e-stream`) — SHIPPED. `delta` events on the job bus (ephemeral, bus-only); the four prose-bearing ops (health synthesis, session-suggest, nutrition check-in, weekly read) reshaped to a shared prose-first reply contract (`prompt/shared.ts` sentinels + `renderStreamingContract` + `extractMarkedJson`); `runChosenStreaming` (streams only when the first agent is stream-capable, falls back to the untouched one-shot rotation on any failure); `jobStreamFilter.ts` buffers until the reply marker so tool narration never reaches a card (no reset event needed); `runOp({stream:true})` paints escaped prose + caret into the anchor, done-render swaps in place. Post-integration review fix: the one-shot (non-streamed) path parses marker-aware too — `RunOpts.extract` threads `extractMarkedJson` through `runChosen`→`runAgentWithFallback`, since a stray `{` in the invited prose anchors plain `extractJson`'s first-brace scan and blanks the parse (threaded as an option because `prompt/shared.ts` imports from `agents.ts` — a direct import would cycle).
