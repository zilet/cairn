...
printf "\n"
printf "  For an installable PWA on your phone, set up Tailscale Serve:\n"
printf "    sudo tailscale serve --bg --https=443 http://127.0.0.1:${PORT}\n"
printf "  Then open https://<pi-name>.<tailnet>.ts.net/ from any signed-in device.\n"
printf "\n"
printf "${YELLOW}${BOLD}  Next: connect one coaching agent${RESET}\n"
printf "\n"
printf "  Log in inside the container (always use -u app):\n"
printf "\n"
printf "  A) Claude Code (Anthropic Pro/Max):\n"
printf "       $COMPOSE exec -u app -it cairn claude\n"
printf "\n"
printf "  B) Codex (ChatGPT):\n"
printf "       $COMPOSE exec -u app -it cairn codex login\n"
printf "\n"
printf "  C) Grok (SuperGrok / X Premium+):\n"
printf "       $COMPOSE exec -u app -it cairn grok -p hello\n"
printf "\n"
