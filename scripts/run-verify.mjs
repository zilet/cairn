...
const clientJobs = [
  {
    ...
    steps: [
      ...
      ["node", "scripts/check-client-build-output.mjs"],
    ],
  },
];
...
async function runGroup(label, jobs) {
  console.log(`Running ${jobs.length} ${label} verification lane${jobs.length === 1 ? "" : "s"} in parallel...`);
