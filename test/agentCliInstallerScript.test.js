...
test("bundled agent manifest pins every supported lazy installer", () => {
  ...
  assert.deepEqual(claude.spec, { method: "npm", package: "@anthropic-ai/claude-code", version: "2.1.258", args: [] });
  ...
  withTempDir((dir) => {
    ...
    executable(
      ...
      [
        ...
        "printf '%s\\n' '#!/bin/sh' 'echo 2.1.258' > \"$CAIRN_CLI_ROOT/bin/claude\"",
    ...
    assert.match(fs.readFileSync(log, "utf8"), /install --global --prefix .*@anthropic-ai\/claude-code@2\.1\.258/);
