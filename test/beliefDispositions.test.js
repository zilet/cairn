...
//   - migration 95 (belief_dispositions) is idempotent
...
// ---------------------------------------------------------------------------
test("migration 95 (belief_dispositions) is idempotent", () => {
  const migration = MIGRATIONS.find((m) => m.version === 95);
  assert.ok(migration, "migration 95 is registered");
  migration.up(db);
