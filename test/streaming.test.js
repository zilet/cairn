...
test("parseChatReply: the reply marker drops an autonomous agent's tool-step narration", () => {
  // antigravity/gemini-style: it narrates each tool step as text before the answer.
  const out = `I will list the directory contents of /app to find the source.
I will query the chat_turns table to check the current turn status.
${CHAT_REPLY_SENTINEL}
Connected and ready — I'm here whenever you want to pick up training.
${CHAT_ACTION_SENTINEL}
{"actions": []}`;
  const { reply, actions } = parseChatReply(out);
  assert.equal(reply, "Connected and ready — I'm here whenever you want to pick up training.");
  assert.deepEqual(actions, []);
});

test("parseChatReply: marker also works with no actions block", () => {
  const out = `Reading the full athlete data from cairn.db to answer accurately.\n${CHAT_REPLY_SENTINEL}\nYour run looks solid — keep the easy pace.`;
  const { reply, actions } = parseChatReply(out);
  assert.equal(reply, "Your run looks solid — keep the easy pace.");
  assert.deepEqual(actions, []);
});

test("parseChatReply: no marker → conservative stripper removes only tool-step lines", () => {
  // Leading lines are unmistakable tool narration (verb + technical token) → dropped;
  // the real coaching answer survives.
  const out = `I will read /app/dist/db.js to understand the schema.\nI'll query the chat_messages table for context.\nGood run today — that 3 km wall is your aerobic base talking.`;
  const { reply } = parseChatReply(out);
  assert.equal(reply, "Good run today — that 3 km wall is your aerobic base talking.");
});

test("parseChatReply: prose that mentions the actions sentinel isn't truncated there", () => {
  // The model explains its own format in prose, then emits the REAL actions block.
  const out = `I'll log that with an ${CHAT_ACTION_SENTINEL} block now.\n${CHAT_ACTION_SENTINEL}\n{"actions": [{"type": "log_activity", "text": "ran 5k"}]}`;
  const { reply, actions } = parseChatReply(out);
  assert.equal(reply, `I'll log that with an ${CHAT_ACTION_SENTINEL} block now.`);
  assert.equal(actions.length, 1);
  assert.equal(actions[0].type, "log_activity");
});

test("parseChatReply: stripper never eats genuine coaching prose", () => {
  // "Let me" / "I'll" with NO technical token must be kept verbatim.
  const out = `Let me explain what your HR zones mean.\nI'll bump your squat to 200 next session.`;
  const { reply } = parseChatReply(out);
  assert.equal(reply, "Let me explain what your HR zones mean.\nI'll bump your squat to 200 next session.");
});
