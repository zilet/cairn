...
async function runChatCompletion(
  ...
  const order: string[] = turn.agent && turn.agent !== "auto" ? [turn.agent] : repo.pickAgentOrder();
  ...
  if (agentSupportsStream(order[0])) {
    ...
    const flush = (final: boolean) => {
      const cut = acc.indexOf(CHAT_ACTION_SENTINEL);
      let safeEnd: number;
      if (cut !== -1) safeEnd = cut;
      else if (final) safeEnd = acc.length;
      else safeEnd = Math.max(emitted, acc.length - (CHAT_ACTION_SENTINEL.length - 1));
      if (safeEnd > emitted) {
