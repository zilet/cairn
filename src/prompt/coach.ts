...
${renderDiscipline(ctx, "training")}${renderEnduranceGoal(ctx, "training")}${renderRunZones(ctx)}${renderRunPlan(ctx)}${renderStrengthSchedule(ctx)}${renderMovementConsiderations(ctx)}${renderConnectedBrain(ctx, { domains: ["training", "watch"] })}${renderTrainingSignals(ctx)}${renderProgramState(ctx)}${renderBodyComp(ctx)}${renderBlock(ctx)}${renderTrajectory(ctx)}${renderTrainingConstraints(ctx)}
