...
interface ResolvedSessionPlanDay {
  day_number: number;
...
function planDayFocus(day: Pick<PlanDayCandidate, "name" | "focus" | "day_number">): string {
  return String(day.focus || day.name || `Day ${day.day_number}`).replace(/\s+/g, " ").trim();
}

function planDayCandidates(): PlanDayCandidate[] {
...
function resolveSessionPlanDay(
  sessionId: number,
...
function nextCandidateAfter(candidates: PlanDayCandidate[], dayNumber: number): PlanDayCandidate {
...
function selectAdaptivePlanDay(date: string): { day_number: number; focus: string | null; selection: Record<string, any> } | null {
