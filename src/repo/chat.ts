...
// Event-driven brain reviews are durable, but the event bus itself is in-memory.
// Deduplicate at the durable boundary too so a restart/re-subscribe or repeated
// equivalent signal cannot spend twice inside the calm cooldown window. The
// fingerprint is generated by brainEvents.ts; this helper validates it again and
// scans only a small recent slice rather than exposing arbitrary JSON/SQL lookup.
export function createBrainReviewAgentJob(
  input: {
    fingerprint: string;
    event: {
      kind: string;
  ...
  const fingerprint = String(input?.fingerprint ?? "")
    .trim()
    .toLowerCase();
  if (!/^[a-f0-9]{64}$/.test(fingerprint)) throw new Error("invalid brain review fingerprint");
  const event = input?.event;
  if (!event || typeof event !== "object") throw new Error("invalid brain review event");
  const date = String(event.date ?? "").slice(0, 10);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) throw new Error("invalid brain review date");
  const boundedCooldown = Math.max(1_000, Math.min(24 * 60 * 60_000, Math.trunc(Number(cooldownMs) || 0)));
  const modifier = `-${Math.ceil(boundedCooldown / 1_000)} seconds`;
  const recent = db
    .prepare(
      `SELECT id, input_json FROM agent_jobs
        WHERE kind = 'brain_review' AND created_at >= datetime('now', ?)
  ...
  for (const row of recent) {
    try {
      ...
      if (parsed?.fingerprint === fingerprint) return { job: getAgentJob(Number(row.id)), created: false };
    } catch {
      // A malformed old telemetry row is ignored; it must not block a valid review.
    }
  }
  ...
  const entityId =
    typeof event.entity_id === "number" && Number.isFinite(event.entity_id)
      ? Math.trunc(event.entity_id)
      : clean(event.entity_id, 120);
  const job = createAgentJob({
    kind: "brain_review",
    phase: "queued for a quiet review",
    input: {
      fingerprint,
      event: {
        kind: clean(event.kind, 80),
        domain: clean(event.domain, 40),
        date,
        entity_id: entityId,
        subject_key: clean(event.subject_key, 160),
        reason: clean(event.reason, 300),
        material: event.material === true,
        clinical: event.clinical === true,
        emitted_at: clean(event.emitted_at, 40),
      },
    },
  });
