...
//     no heart rate for — a constant 65.534 marked `isAutoCalcCalories`. That is a
//     placeholder, not a measurement, and summing it into the day claims an energy
//     cost nobody paid.
//
 ...
 * estimate is still theirs and is kept. On a shell CAIRN created it is a number
 * nobody produced — Garmin filling a required field for a payload we invented — so it
 * reads as absent. The sentinel value is treated as absent wherever it appears: it is
 * an encoding artifact, never a real 65.534 kcal.
 */
export function garminActivityCalories(
  ...
  if (opts.cairnAuthored && activity?.isAutoCalcCalories === true) return null;
  return calories;
}
