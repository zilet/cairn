...
function doseExerciseIdentity(name: string): {
  ...
  const alias = norm
    ? (db.prepare(`SELECT canonical FROM exercise_aliases WHERE alias = ?`).get(norm) as any)
    : null;
...
function canonicalDoseSets(
  rows: DoseSet[],
  providerImport: boolean
): { sets: DoseSet[]; duplicates: number } {
  const kept: DoseSet[] = [];
  ...
  for (const row of rows) {
    ...
    if (
      prior != null &&
      prior.exercise_id !== row.exercise_id &&
      providerImport &&
      canonicalProviderPair(prior, row)
    ) {
      duplicates++;
      continue;
    }
...
export function recoverySessionDose(sessionId: number): RecoverySessionDose {
  ...
  ).all(sessionId) as any[]).map((row): DoseSet => {
  ...
  const createdAt = String(session?.created_at ?? "").replace("T", " ").slice(0, 19);
  const stampedAt = String(ledger?.stamped_at ?? "").replace("T", " ").slice(0, 19);
  const predatesSnapshot = !!(
    ledger &&
    createdAt &&
    stampedAt &&
    createdAt < stampedAt
  ...
  const rirs = working.map((row) => row.rir).filter((value): value is number => value != null && Number.isFinite(value));
  const medRir = median(rirs);
  const nearFailure = rirs.filter((rir) => rir <= 2).length;
  const targetRatios = working.map((row) => {
  ...
  if (plannedSets != null && plannedSets > 0) {
    ...
    const loadOverrun = (maxTargetRatio != null && maxTargetRatio > 1.3) ||
      targetRatios.filter((ratio) => ratio > 1.15).length >= 2;
    const poorFeedback =
    ...
    if (canonicalSets >= overdoseLimit || hardEffort || loadOverrun) {
      ...
      reason = canonicalSets >= overdoseLimit
...
export function cardioEffort(a: {
  ...
  const trainingEffect = Math.max(Number(a.training_effect) || 0, Number(a.aerobic_te) || 0, Number(a.anaerobic_te) || 0);
  const label = String(a.te_label || "").toLowerCase();
  if (
    trainingEffect >= 4 ||
    /\b(?:vo2(?:[\s_-]*max)?|maximal|anaerobic|sprint|interval|threshold)\b/.test(label)
  ) return "hard";
...
export function dayLoad(date: string, opts: { countsCardio: boolean; recoveryWeekActive?: boolean }): TrainingLoad | "none" {
