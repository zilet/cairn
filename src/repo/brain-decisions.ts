...
export function saveBrainRollback(
  decisionId: number,
  kind: "training_plan" | "nutrition_target",
  payload: unknown
): boolean {
...
export function getBrainRollback(
  decisionId: number
): { kind: "training_plan" | "nutrition_target"; payload: any } | null {
  const row = db.prepare(`SELECT kind, payload_json FROM brain_rollbacks WHERE decision_id = ?`).get(decisionId) as any;
  if (!row || !["training_plan", "nutrition_target"].includes(String(row.kind))) return null;
  try {
