...
export function buildMcpServer(): McpServer {
  ...
  server.tool("set_profile", "Update profile fields (any subset). Weight in lb, height in cm. about_me is free-text the coach uses to personalize (training history, work pattern, food likes/dislikes, what 'better' means to you); pass '' to clear.",
    {
  ...
  server.tool("list_memory", "List accumulated memory notes.", { limit: z.number().int().optional() },
    async ({ limit }) => asText(repo.listMemory(limit ?? 50)));

  server.tool("update_memory",
    "Edit an existing memory note's content and/or kind by id.",
    { id: z.number().int(), content: z.string().optional(), kind: z.string().optional() },
    async ({ id, content, kind }) => asText(repo.updateMemory(id, { content, kind }) ?? { error: "not found", id }));
  ...
  server.tool("set_settings",
    "Update app settings (any subset). agent_strategy: round_robin|random|priority. agent_order / disabled_agents: arrays of agent names. coach_enabled/coach_day(0-6)/coach_hour(0-23) control the weekly auto-draft.",
    {
  ...
  server.tool(
    "add_family",
    "Add a family member to the roster. relationship is e.g. son / daughter / partner / parent; color is an optional swatch; birthdate is optional YYYY-MM-DD; notes is free-text.",
    {
      name: z.string(),
  ...
  server.tool(
    "update_family",
    "Update a family member by id (any subset of fields).",
    {
      id: z.number().int(),
      name: z.string().nullable().optional(),
      color: z.string().nullable().optional(),
