...
export function registerConnectedBrainTools(server: McpToolRegistrar) {
  server.tool(
    ...
    "Read recent meaningful coaching decisions and their accountable status. Bounded, with no raw prompts or hidden reasoning.",
  ...
  server.tool(
    "get_health_standing",
    "A health standing read: actual-age vs selectable reference-age percentiles for markers with real reference curves (VO2max/body composition), plus BP, labs, activity, Garmin/recovery signals, and a plain signal_age synthesis. Motivational orientation only.",
    {
      reference_age: z
        .number()
        .optional()
  ...
  server.tool(
    ...
    "Cardiovascular risk input/enhancer read: collects PREVENT/PCE inputs, names missing clinical inputs, surfaces ApoB/Lp(a)/hs-CRP/body-fat/VO2max/family-history risk enhancers, and gives counterfactual lever projections. It does not emit a risk percentage unless sourced coefficients are vendored.",
    {},
  ...
  server.tool(
    "get_doctor_loop",
    "Doctor-loop read: structured missing-workup recommendations plus lab/DEXA retest attention derived through the adaptive attention engine. No PREVENT/PCE risk coefficients are computed here.",
    {},
  ...
  server.tool(
    "get_doctor_packet",
    "Export-ready doctor packet: current prioritized health focus, active connected-brain directives, doctor-loop missing-workup/retest plan, AHA PREVENT cardiovascular-risk read, and latest intervention-outcome annotations.",
  ...
  server.tool(
    "run_health_review",
    "Queue a durable whole-picture health review over the user's context and marker history. Returns a job immediately; poll get_agent_job.",
  ...
  server.tool(
    "get_health_focus",
    "The deterministic TIERED health priorities (elite-coach prioritization): the flat directive flood collapsed into a handful of connected priorities — one per health group, tier (act_now/track), the markers driving it, whether they compound, and the LEAD move per domain. The basis for the whole-picture synthesis.",
    {},
  ...
  server.tool(
    "get_trajectory",
    "The user's forward ARC: a periodized horizon (weeks/phase) toward their goals (body-comp, longevity markers, any race), the milestones along it, and today framed as the next step on the path. No completion %; null line when there's no goal/block/race.",
  ...
  server.tool(
    ...
    "Read the standing everything-better objective across strength, endurance, body composition, metabolic health, and recovery. Names what this phase intentionally parks and flags only unexplained regression.",
    {
  ...
  server.tool(
    "get_context_effect",
    "The active life-context effect: events the user mentioned once (a late concert, travel, illness, a hard week) that should shape today — expect worse sleep / a transient inflammation bump (don't alarm) / ease the load / disrupted fueling — each with a fade date. Empty when nothing's active.",
    { date: z.string().optional() },
  ...
  server.tool(
    "get_next_step",
    "The single highest-leverage next action across ALL domains (train/fuel/recover/recheck/life) right now — one calm thing, or null on a quiet day.",
    { date: z.string().optional() },
  ...
  server.tool(
    "mark_next_step",
    "Record the user's response to a next-step: action 'done' (did it) or 'snooze' (not today) by step_key — so a handled/skipped step doesn't return tomorrow.",
  ...
  server.tool(
    "synthesize_health",
    "Queue and cache the elite-coach whole-picture synthesis across labs, body composition, training, recovery, nutrition, supplements, and life. Returns a job immediately; poll get_agent_job.",
  ...
  server.tool(
    "research",
    "Queue one cited health/longevity evidence pass and cache its verified claims. Returns a job immediately; poll get_agent_job. Gated by research_enabled; off means no network.",
    {
  ...
  server.tool(
    "get_evidence",
    "Make a directive's citation INSPECTABLE: returns the cited evidence behind ONE marker as { marker, evidence:[{claim, source_title, source_url, body, confidence, retrieved_at}] }. Reads the evidence cache only (never the network), so it works with research disabled; evidence:[] when research never ran for that marker.",
    {
      marker: z
        .string()
        .optional()
  ...
  server.tool(
    ...
    "Make the evidence cache DISCOVERABLE without an N-fetch fan-out: returns { research_enabled, total, by_marker:[{marker, count}] } — the per-marker count of cited rows on file, so a UI can show a 'see the evidence (N)' hint and know up-front where evidence exists. Reads the cache only (never the network).",
    {},
  ...
  server.tool(
    ...
    "The quiet 'What Cairn has noticed' read: durable, plain-language learnings drawn from suggestion → actual reconciliation (e.g. 'tolerates higher training frequency than the read assumed'). Returns { learnings:[{id, content, noticed_at}] }, newest-first. These season the coach's defaults.",
    { limit: z.number().int().optional() },
