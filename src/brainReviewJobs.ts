  ...
  if (stored.created) {
    ...
    queueMicrotask(() => {
      try {
        ...
        log.error(`[brain] could not enqueue review job#${stored.job.id} (${diagnosticErrorName(error)})`);
      }
    });
  }
  return stored;
...
export function startBrainReviewJobSubscriber(deps: BrainReviewSubscriberDeps = {}): () => void {
  ...
  const off = onBrainEvent((event) => {
    try {
      ...
      log.error(`[brain] signal review skipped safely (${diagnosticErrorName(error)})`);
    }
  });
  unsubscribe = () => {
    off();
