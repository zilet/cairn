...
type VolumeBalanceGroup = {
  group?: unknown;
};

type VolumeBalance = {
  groups?: VolumeBalanceGroup[];
  due?: unknown[];
  over?: unknown[];
  broad_low?: unknown;
  summary?: unknown;
...
const VOLUME_PATTERN_WORD: Record<string, string> = { core: "core", forearms: "grip" };

function capWord(input: unknown): string {
  const value = String(input || "");
  return value ? value.charAt(0).toUpperCase() + value.slice(1) : value;
}

function volBalanceHtml(balance: VolumeBalance | null | undefined): string {
  if (!balance || !Array.isArray(balance.groups) || !balance.groups.length) return "";
  ...
  const chip = (label: unknown, cls: string): string => `<span class="vbal-chip ${cls}">${escHtml(label)}</span>`;
  const dueShown = balance.broad_low ? due.slice(0, 4) : due;
  const dueMore = due.length - dueShown.length;
  const dueChips =
    dueShown.map((group) => chip(capWord(group), "vbal-due")).join("") +
