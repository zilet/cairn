...
type PlanEditorInput = HTMLInputElement | HTMLTextAreaElement;
...
function planInput(el: Element | null | undefined): PlanEditorInput | null {
...
function planDayNumber(day: PlanEditorModelDay): number {
...
async function renderPlanEditor(): Promise<void> {
  ...
  const peek = peekCached<PlanEditorApiDay[]>("plan");
  ...
  const model: PlanEditorModelDay[] = (Array.isArray(plan) ? plan : []).map((day) => dayModelFromPlan(day));
