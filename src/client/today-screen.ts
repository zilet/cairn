...
function exTimed(it: any, logged: any) {
  if (it.mode === "timed" || it.target_seconds != null) return true;
  if ((todayState.exModes || {})[it.exercise] === "timed") return true;
  return (logged || []).some((s: any) => s.duration_sec != null);
}
...
function todayRxTargetText(rx: any) {
...
function todayExRxVaryMenuHtml(rx: any) {
...
function cardioDoneCard(it: any, eff: any, revealIdx: any) {
  const label = cardioLabel(it);
  ...
  const chipHtml = chips.map((c: any) => `<span class="done-chip">${escHtml(c)}</span>`).join("");
...
function todayCardioDominantZone(zones: any) {
...
function todayCardioVerb(label: any) {
...
function todayCardioLogPhrase(it: any) {
...
function sessionSuggestOpOpts() {
  return {
    ...
    guard: () => { const gone = !todayView.querySelector("#sugSlot")?.isConnected; if (gone) sessionSuggestInFlight = false; return gone; },
    isFail: (r: any) => !r || r.ok !== true || !r.session,
    render: (r: any) => {
...
function reconnectSessionSuggest() {
  ...
  const clear = () => { stop(); const s = todayView.querySelector("#sugSlot"); if (s) { s.classList.remove("is-thinking", "is-thinking--determinate"); s.style.removeProperty("--frac"); } };
  return {
    guard: o.guard,
    onDone: (result: any) => { clear(); if (o.isFail(result)) o.onFail(result); else o.render(result); },
...
function revealSessionComposer() {
  ...
  slot.querySelectorAll("[data-vibe]").forEach((b: any) => b.addEventListener("click", () => {
  ...
  input?.addEventListener("keydown", (e: any) => { if (e.key === "Enter") { e.preventDefault(); go(); } });
}
...
async function askForSession(opts: any = {}) {
  ...
  slot.innerHTML = CairnTodaySessionSuggest.loadingHtml();
  const body: any = { date: todayState.logDate };
...
function wireSuggestCard(slot: any) {
  slot.querySelectorAll("[data-sugaction]").forEach((b: any) =>
    b.addEventListener("click", () => {
      ...
      if (act === "log") {
        ...
        if (card && todayView.querySelector(".addex")) collapseEl(card, handoff); else handoff();
      }
    })
  );
}
...
async function renderToday(opts: any = {}) {
  ...
  // ---- Plan / logging surface — the launchpad, shown when the day calls for it ----
    ...
    html += skipLineHtml(skippedItems.map((it: any) => (isCardioItem(it) ? cardioLabel(it) : it.exercise)));
  ...
  if (finishBtn) finishBtn.addEventListener("click", async () => {
    ...
    const settle = () => {
      if (todayState.tab !== "today") return; // user navigated away during the exit animation
  ...
  if (reopenBtn) reopenBtn.addEventListener("click", async () => {
    ...
    try { await todayApi(`/sessions/${session.id}/reopen`, { method: "POST" }); } catch {}
  ...
  todayView.querySelectorAll(".ex .logrow").forEach((row: any) => wireLogRow(row));

  if (hasLoggedSets) renderFeedback(todayView.querySelector("#feedbackSlot"), session);
...
function hasFeedback(session: any) {
  return CairnTodaySessionStatus.hasFeedback(session);
}

function renderFeedback(slot: any, session: any) {
  if (!slot) return;
...
function renderFeedbackForm(slot: any, session: any) {
  ...
  slot.querySelectorAll(".feel-dot").forEach((b: any) =>
    b.addEventListener("click", async () => {
      const kind = b.dataset.feel, val = Number(b.dataset.val);
      picked[kind] = val;
      slot.querySelectorAll(`.feel-dot[data-feel="${kind}"]`).forEach((d: any) =>
...
function renderFeedbackDone(slot: any, session: any) {
...
function wireDeletes() {
  todayView.querySelectorAll("[data-del]").forEach((b: any) => {
    if (b._wired) return; b._wired = true;
...
function collapseEl(el: any, done: any) {
  el._collapsed = true;
...
function addSkipName(name: any) {
  ...
  const dup = [...names.querySelectorAll("[data-unskip]")]
    .some((b: any) => decodeURIComponent(b.dataset.unskip).toLowerCase() === name.toLowerCase());
  if (!dup) {
...
function removeSkipName(name: any) {
  const line = todayView.querySelector("#skipLine");
  if (!line) return;
  [...line.querySelectorAll("[data-unskip]")]
    .filter((b: any) => decodeURIComponent(b.dataset.unskip).toLowerCase() === name.toLowerCase())
    .forEach((b: any) => b.remove());
  if (!line.querySelector("[data-unskip]")) line.classList.add("skipline-empty");
}

async function skipFromCard(card: any, exercise: any) {
  let res;
  try {
...
async function undoSkip(card: any, anchor: any, exercise: any) {
  try {
...
function removeOffPlanCard(card: any) {
  if (!card) return;
  ...
  if (name && todayState.pendingOffPlan && todayState.pendingOffPlan[todayState.logDate]) {
    todayState.pendingOffPlan[todayState.logDate] = todayState.pendingOffPlan[todayState.logDate].filter(
...
function wireSkips() {
  todayView.querySelectorAll(".ex-skip").forEach((b: any) => {
    if (b._wired) return; b._wired = true;
  ...
  if (line && !line._wired) {
    line._wired = true;
    line.addEventListener("click", async (e: any) => {
...
let _rxRefreshTimer: ReturnType<typeof setTimeout> | null = null;
function scheduleRxRefresh() {
  if (todayState.day == null) return; // a pure run/rest day has no per-lift progression
  if (_rxRefreshTimer != null) clearTimeout(_rxRefreshTimer);
  _rxRefreshTimer = setTimeout(() => { refreshAdaptedRx(); }, 600);
}
function invalidateTodayProgression() {
  if (todayState.day != null) swrInvalidate("program:progression:" + todayState.day);
}
async function refreshAdaptedRx() {
  if (todayState.tab !== "today" || todayState.day == null) return;
  ...
  if (todayState.tab !== "today" || todayState.day !== day || todayState.logDate !== date) return;
  if (!Array.isArray(list)) return;
  const rxByEx: Record<string, any> = {};
  ...
  todayView.querySelectorAll(".ex[data-card]").forEach((card: any) => {
...
function bumpProgress(card: any) {
...
async function loadRecentActivities() {
  ...
  wrap.querySelectorAll('.lately-head[role="button"]').forEach((h: any) => {
    const toggle = () => {
...
async function loadWearable(isToday: any) {
  ...
  let rows = [];
  try { rows = await todayApi("/garmin/daily?limit=1"); } catch { return; }
...
function isNearTermContext(ev: any) {
  ...
  if (start && start > today) return (daysUntil(start) ?? Infinity) <= CTX_NEAR_DAYS; // imminent
  return false; // undated or far-future non-injury → not on Today
}

function ctxBannerLine(ev: any) {
...
// future goal date, and never on a past-date todayView.
function goalLineHtml(stats: any, curW: any, isToday: any) {
...
async function loadDraftProposals() {
  ...
  if (!slot) return;
  let plans = [];
  try { plans = await todayApi("/proposals?limit=8"); } catch { return; }
  if (todayState.tab !== "today" || !slot.isConnected) return;
...
function fuelCardHtml(d: any) {
  return window.CairnTodayAgenda.fuelCardHtml(d);
}

async function loadFuelToday(date: any) {
  const slot = todayView.querySelector("#fuelSlot");
  if (!slot) return;
  let d = null;
  try { d = await todayApi(`/nutrition/day?date=${encodeURIComponent(date || todayState.logDate)}`); } catch { return; }
  if (todayState.tab !== "today" || !slot.isConnected) return;
  ...
  if (card) card.addEventListener("click", () => { todayState.planJump = "food"; activateTab("plan"); });
  runCountUps(slot);
}
...
async function loadProgramAdjustmentsBanner() {
  ...
  if (!slot) return;
  let rows = null;
  try { rows = await todayApi("/program/adjustments"); } catch { rows = null; }
  if (todayState.tab !== "today" || !slot.isConnected) return;
...
async function loadGarminReconcile() {
  ...
  if (!slot) return;
  let rows = [];
  try { rows = await todayApi("/garmin/unreconciled"); } catch { rows = []; }
  if (todayState.tab !== "today" || !slot.isConnected) return;
  rows = Array.isArray(rows) ? rows : [];
  if (!rows.length) { slot.innerHTML = ""; return; }
  const n = rows.length;
