...
function createMeHealthScreenComposition(input: ClientMeHealthScreenCompositionInput): ClientMeHealthScreenComposition {
  const goStand = (): void => {
    (globalThis as unknown as { activateTab?: (tab: string) => void }).activateTab?.("stand");
  };
  ...
  const screen: ClientMeHealthScreenComposition = {
    ...
    renderMe: () => {
      ...
      const titles: Record<string, string> = { profile: "Profile", memory: "Memory", life: "Life", family: "Family" };
