...
type ProgressLineChartModelOptions = {
  width: number;
  height: number;
  goal?: number | null;
...
function progressLineChartPadding(
  padding: Partial<ProgressLineChartPadding> | null | undefined,
): ProgressLineChartPadding {
  return {
...
function progressLineChartDomain(values: number[], goal: number | null | undefined): { min: number; max: number } {
...
function progressLineChartSlopes(xs: number[], ys: number[]): number[] {
  const count = ys.length;
  const slopes = new Array<number>(count).fill(0);
  if (count <= 1) return slopes;
...
function progressLineChartPeakIndex(values: number[]): number {
  let peakIndex = 0;
...
function buildProgressLineChartModel(
  pts: ProgressChartPoint[] | null | undefined,
  options: ProgressLineChartModelOptions,
  ...
  const y = (value: number) =>
    height - padding.bottom - ((value - min) / (max - min)) * (height - padding.top - padding.bottom);
...
function nearestProgressLineChartIndex(axis: readonly number[] | null | undefined, pixelX: number): number | null {
