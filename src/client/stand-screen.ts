...
(() => {
  ...
  const DOMAINS: StandDomain[] = [
    ...
    {
      key: "screening",
      label: "Screening & Other",
      groups: ["autoimmune", "infectious", "screening", "metals", "urinalysis", "other"],
    },
  ];
  const HM = () =>
    (globalThis as unknown as { CairnHealthMarkers?: Record<string, (...a: unknown[]) => unknown> }).CairnHealthMarkers;
  const BM = () =>
    (globalThis as unknown as { CairnBodyMetrics?: Record<string, (...a: unknown[]) => unknown> }).CairnBodyMetrics;
  ...
  const SELF_CONTAINED: ReadonlySet<StandView> = new Set([
    "records",
    "share",
    "learned",
    "connections",
  ...
  const WHOLE_PICTURE_Q =
    "Walk me through my whole health picture — what matters most right now, and the single most effective thing I can do about it?";
  ...
  function readHtml(): string {
    ...
    const zones = prios
      ...
      .join("");
    const oc =
      syn && typeof syn.one_change === "string" && syn.one_change.trim()
  ...
  function bodyTile(): string {
    ...
    const st = bodyStatus();
    const read =
      whtr && whtr.value != null ? `waist <b>${escHtml(String(whtr.value))}</b> of height` : "log a tape session";
  ...
  // itself honestly).
  function recoveryIsFresh(): boolean {
    const rec = recoveryData();
  ...
  function pictureDeps(): ClientHealthPictureControllerDeps {
    return {
      ...
      switchHealthSeg: (seg, opts) => {
        if (seg === "records") showRecords(opts || {});
        else showOverview();
      },
  ...
  function readDeps(): ClientHealthReadControllerDeps {
    return {
      ...
      reducedMotion,
      switchHealthSeg: (seg) => {
        if (seg === "markers") showAllMarkers();
        else showOverview();
      },
      isHealthReviewRunning: () => CairnHealthPictureController.isHealthReviewRunning(),
      loadHealthPicture: (token, docsPromise) =>
        CairnHealthPictureController.loadHealthPicture(token, docsPromise, pictureDeps()),
      paintHealthPicture: () => CairnHealthPictureController.paintHealthPicture(pictureDeps()),
      setReadSpy: () => {},
      teardownReadSpy: () => {},
    };
  ...
  function showLearned(): void {
    ...
    cachedApi("/learned-timeline", {
      ...
      onUpgrade: (data, { changed }) => {
        if (changed || !peek) paintLearned(data, token);
      },
    }).catch(() => {
  ...
  function bodyDetailHtml(): string {
    ...
    const dexa = markersOfGroup("body")
      .map((m) => HM()?.hmkRowHtml?.(m) as string)
      .filter(Boolean)
      .join("");
    return `<div class="stand-detail stand-root">
      <button class="stand-back linkbtn linkbtn-plain" data-back>‹ Stand</button>
  ...
  function showBody(): void {
    ...
    (BM()?.renderBodyMetrics as ((m: HTMLElement | null) => void) | undefined)?.(
      view.querySelector<HTMLElement>("#standBodyMetrics")
    );
    wireRows(view);
  }
  function showRecovery(): void {
    curView = "recovery";
    setStandSeg("recovery");
  ...
  function wireRows(wrap: ParentNode): void {
    ...
    wrap
      .querySelectorAll<SVGElement>("svg.hchart")
      .forEach((svg) => (HM()?.wireMarkerChart as ((s: SVGElement) => void) | undefined)?.(svg));
  ...
  async function fetchStandData(): Promise<StandData> {
    const [priority, focus, body, synthRes, insightsRes, recoveryRes, suppRes, dirRes, checkupRes] = await Promise.all([
      ...
      (api("/health/synthesis") as unknown as Promise<{ synthesis?: StandSynthesis; stale?: unknown }>).catch(
        () => null
      ),
    ...
    return {
      ...
      synthStale: !!(
        synthRes &&
        typeof synthRes === "object" &&
        (synthRes.stale ?? (syn as { stale?: unknown } | null)?.stale)
      ),
      ...
      supplements: (Array.isArray(suppRes)
        ? suppRes
        : (suppRes as { supplements?: unknown[] } | null)?.supplements || []
      ).filter(
        (s): s is Record<string, unknown> => !!s && typeof s === "object" && (s as { active?: unknown }).active !== 0
      ),
  ...
  async function renderStand(): Promise<void> {
    ...
    if (seg && SELF_CONTAINED.has(seg as StandView)) {
      if (DATA) void revalidateStand();
      else void loadStandData();
