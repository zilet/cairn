...
function compSection(data: BmSummary, unit: BmUnit): string {
  ...
  const waistIn = m.merged?.waist_in != null ? (unit === "cm" ? (m.merged.waist_in as number) / 2.54 : (m.merged.waist_in as number)) : null;
  ...
  else if (inBand) legendBits.push(`<span style="color:var(--sage-text,#5f6e4f)">waist inside the optimal band for your height</span>`);
