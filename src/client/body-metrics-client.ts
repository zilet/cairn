...
function zoneRow(s: BmScale): string {
...
const BM_FIG_REG: Record<string, Array<[number, number, number, number]>> = {
  waist: [[60, 110, 26, 25]],
  chest: [[60, 80, 33, 19]],
...
function bodyFigureSvg(focus: string | null, wins: string[]): string {
  const ACCENT = "#b4552d", SAGE = "#6e7f5c", SAGE_DEEP = "#5a6a4a";
  const grad = (id: string, c: string) =>
    `<radialGradient id="${id}" cx="50%" cy="50%" r="50%"><stop offset="0%" stop-color="${c}" stop-opacity="0.9"/><stop offset="45%" stop-color="${c}" stop-opacity="0.42"/><stop offset="100%" stop-color="${c}" stop-opacity="0"/></radialGradient>`;
  let grads = "", glows = "", marks = "";
  const region = (key: string, color: string, tag: string) => {
  ...
  if (focus) region(focus, ACCENT, "f");
  for (const k of wins) if (k !== focus) region(k, SAGE, "w");
  for (const k of wins) if (k !== focus) for (const g of BM_FIG_REG[k] || [])
...
function compSection(comp: BmComp | undefined, trends?: BmSummary["trends"]): string {
  if (!comp) return "";
...
function logForm(sites: { key: string; label: string; hint?: string }[], unit: BmUnit): string {
...
function unitToggle(unit: BmUnit): string {
  const btn = (u: BmUnit) =>
    `<button type="button" class="chip bm-unit-btn" data-unit="${u}" aria-pressed="${u === unit}" style="padding:2px 10px;font-size:.74rem${
      u === unit ? ";background:var(--ink,#2f2b23);color:var(--card,#faf6ec);border-color:var(--ink,#2f2b23)" : ""
    }">${u}</button>`;
