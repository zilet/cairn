...
async function activeDirectives(): Promise<CaptureDirective[]> {
  ...
  try {
    ...
    rows = (res && Array.isArray(res.directives) ? res.directives : []).filter(
      (d): d is CaptureDirective => !d.status || d.status === "active",
    );
  } catch { rows = []; }
...
function provenanceLineHtml(d: CaptureDirective | null | undefined, label: string): string | null {
  if (!d) return null;
...
function wireProvenance(scope?: ParentNode | null): void {
  (scope || view).querySelectorAll<HTMLElement>("[data-prov]").forEach((b) => b.addEventListener("click", () => {
    state.meSeg = "health";
...
const SpeechRec: CaptureSpeechRecognitionCtor | null =
  (window as Window & {
    SpeechRecognition?: CaptureSpeechRecognitionCtor;
    webkitSpeechRecognition?: CaptureSpeechRecognitionCtor;
  }).SpeechRecognition
  || (window as Window & { webkitSpeechRecognition?: CaptureSpeechRecognitionCtor }).webkitSpeechRecognition
  || null;
let _voiceRec: CaptureSpeechRecognition | null = null; // live recognition instance while listening
...
function setupVoiceCapture(): void {
  const mic = view.querySelector<HTMLElement>("#qlMic");
  const inp = view.querySelector<HTMLInputElement>("#qlInput");
  if (!mic || !inp) return;
