...
function chatMessageRecord(value: unknown): Record<string, unknown> {
  return value && typeof value === "object" ? value as Record<string, unknown> : {};
}

function chatMessageRows<T extends Record<string, unknown> = Record<string, unknown>>(value: unknown): T[] {
  return Array.isArray(value) ? value.filter((row): row is T => !!row && typeof row === "object") : [];
}

function chatMessageMeta(value: unknown): ChatScreenMeta {
  return chatMessageRecord(value) as ChatScreenMeta;
}

function chatMessageApplied(value: unknown): ChatScreenAppliedAction[] {
  return chatMessageRows<ChatScreenAppliedAction>(value);
}

function chatMessageDrafts(value: unknown): ChatScreenDraft[] {
  return chatMessageRows<ChatScreenDraft>(value);
}

function chatMessageAgentAttempts(value: unknown): Array<Record<string, unknown>> {
  return chatMessageRows<Record<string, unknown>>(value);
}

function chatMessageString(value: unknown): string {
  return value == null ? "" : String(value);
}

function chatAgentCheckHtml(attemptsValue: unknown): string {
...
function chatMessageDayISO(ts: unknown): string {
...
function appendMsg(
  ...
  if (m.pending) {
    ...
    host.appendChild(el);
    if (!noScroll && log && stickBottom) log.scrollTop = log.scrollHeight;
    return el;
  }
  ...
  el.querySelectorAll("[data-agent-check]").forEach((b) => {
    const btn = b instanceof HTMLButtonElement ? b : null;
    if (btn) btn.addEventListener("click", openAgentSettings);
  });
