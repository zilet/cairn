...
async function renderProgress() {
  ...
  return paintSWR({
    ...
    token,
    tab: "progress",
    render: (exercises: unknown) => paintProgressBody(CairnProgressData.rows<ProgressExercise>(exercises)),
  });
}
...
async function renderWeight() {
  ...
  const paint = (rows: unknown, profile: unknown) => {
    ...
    paintWeightBody(CairnProgressData.rows<ProgressWeightRow>(rows), CairnProgressData.record(profile));
  };
