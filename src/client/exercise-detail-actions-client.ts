...
function exerciseDetailActionRecord(value: unknown): ExerciseDetailActionRecord {
  return value && typeof value === "object" ? value as ExerciseDetailActionRecord : {};
}

function wireExerciseDetailActions(
  el: ParentNode,
  row: ExerciseDetailActionRow,
  fallbackName: string,
  timed: boolean,
  deps: ExerciseDetailActionDeps,
): void {
  ...
  if (typeBtn) typeBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (deps.state.exModes) deps.state.exModes[displayName] = next;
  ...
  if (deleteBtn) deleteBtn.addEventListener("click", async () => {
    ...
    let result: ExerciseDetailActionRecord;
    try {
