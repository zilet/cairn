...
type CairnMotionElement = HTMLElement & {
...
function clearMotionStyles(el: CairnMotionElement): void {
...
function collapseEl(input: Element | null | undefined, done?: CollapseDone): void {
  ...
  el._animTimer = setTimeout(() => {
    if (el._collapsed && done) done();
  }, 380);
}

function expandEl(input: Element | null | undefined): void {
  const el = motionElement(input);
  if (!el) return;
  clearTimeout(el._animTimer);
  el._collapsed = false;
