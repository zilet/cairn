...
function createCaptureReadsController(deps: CaptureReadsDeps): CaptureReadsController {
  ...
  function renderInsightInSlot(target: HTMLElement, ins: CaptureInsight): void {
  ...
  function renderInsightCard(target: HTMLElement, ins: CaptureInsight): void {
    ...
    target.querySelectorAll<HTMLElement>("[data-ifb]").forEach((button) =>
      button.addEventListener("click", () => insightFeedback(target, ins, button.dataset.ifb)));
  ...
  function renderWeeklyInSlot(target: HTMLElement, ins: CaptureInsight): void {
    if (!target || !ins) return;
  ...
  function renderWeeklyCard(target: HTMLElement, ins: CaptureInsight): void {
    const text = deps.escapeHtml(String(ins.text || ""));
    ...
    target.querySelectorAll<HTMLElement>("[data-ifb]").forEach((button) =>
      button.addEventListener("click", () => insightFeedback(target, ins, button.dataset.ifb, ".weekly-card")));
  ...
  function wireWhyToggle(target: HTMLElement, bodySelector: string): void {
