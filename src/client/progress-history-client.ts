...
function sessionSetScore(set: ProgressHistorySet): number {
...
async function renderHistory() {
  ...
  return paintSWR({
    ...
    token,
    tab: "progress",
    render: (sessions: unknown) => paintHistoryBody(progressHistoryRows<HistorySession>(sessions)),
  });
}
...
async function openSessionEdit(sess: HistorySession, fromEl: Element) {
  ...
  const byEx: Record<string, ProgressHistorySet[]> = {};
  for (const s of sets) {
