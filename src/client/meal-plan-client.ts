...
(() => {
  ...
  function mealRecord(value: unknown): MealRecord {
    return value && typeof value === "object" ? value as MealRecord : {};
  }
  ...
  function mealsCtxFor(plan: unknown, now?: unknown): MealPlannerContext {
  ...
  function mealRowHtml(meal: unknown, mealIndex?: number, options?: MealRowOptions): string {
  ...
  function mealDayHtml(day: unknown, dayIndex: number, context: MealDayContext): string {
    const d = mealRecord(day);
