...
type ClientActivity = import("../contracts/client.js").ClientActivity;
...
function actArtText(activity: ClientActivityLike): string {
  const type = String(activity.type || "").toLowerCase();
  for (const key in ACT_ART_PHRASE) if (type.includes(key)) return ACT_ART_PHRASE[key];
  return String(activity.type || activity.raw_text || "");
}

function actEntryHtml(activity: ClientActivityLike): string {
...
function updateActEntry(el: Element, row: ClientActivityLike): void {
  const line = el.querySelector(".qlent-line");
  if (line) line.textContent = activityLine(row);
  const badge = el.querySelector(".qlent-badge");
  if (badge) badge.innerHTML = enrichBadge(row.enrichment_status);
