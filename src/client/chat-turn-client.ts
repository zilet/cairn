...
(() => {
  ...
  const chatTurnMonitor = root.CairnChatTurnMonitor.create({
    ...
    pendingIds: () => [...chatPendingBubbles.keys()].filter((id) => chatPendingBubbles.get(id)?.isConnected),
    createStream: (id) =>
  ...
  function makePendingBubble(turn: ChatTurn): HTMLElement {
    ...
    el.innerHTML =
      `<div class="bubble-text"><span class="typing-cap">${escHtml(cap)} </span>` +
  ...
  function finalizeTurn(turnValue: unknown, messageValue?: unknown): void {
    ...
    const applied = Array.isArray(chatTurnRecord(row.meta).applied)
      ? (chatTurnRecord(row.meta).applied as Array<Record<string, unknown>>)
      : [];
    // Cache freshness follows confirmed persistence, not the stronger prose receipt.
    // A write may commit while full readback verification is unavailable; the server
  ...
  async function cancelTurn(id: number): Promise<void> {
    ...
    if (chatTurnMonitor.currentId() === id) chatTurnMonitor.close();
    chatMonitorEnsure();
  }
