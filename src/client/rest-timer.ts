...
const rest: RestTimerState = { id: null, remaining: 0, total: 0 };
...
function paintRest(): void {
  ...
  if (time) time.textContent = `Rest ${minutes}:${seconds}`;
  const fill = bar.querySelector<HTMLElement>(".rest-fill");
  if (fill) fill.style.width = `${Math.max(0, (rest.remaining / rest.total) * 100)}%`;
}
