...
function getInstallGuidance(): InstallGuidance | null {
  ...
  if (deferredInstallPrompt) return { mode: "prompt" };
  ...
  if (isMacSafari) return { mode: "safari-desktop" };
  if (isChromium) return { mode: "chromium-menu" };
  if (isAndroid) return { mode: "menu-generic" };
  return null;
}

function phoneCoachContent(mode: InstallGuidance["mode"]): string {
...
function wirePhoneCoach(el: Element): void {
  ...
  const installBtn = el.querySelector<HTMLButtonElement>(".phone-coach-install");
  if (installBtn) {
...
function refreshPhoneCoach(): void {
  try {
    document.querySelectorAll<HTMLElement>(".phone-coach").forEach((el) => {
      ...
      if (el.dataset.coachMode === guidance.mode) return;
      el.dataset.coachMode = guidance.mode;
      el.innerHTML = phoneCoachContent(guidance.mode);
      wirePhoneCoach(el);
    });
