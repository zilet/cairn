...
export const CHAT_ACTION_PROMPT_SPECS = {
  ...
  flag_training_structure: {
    ...
    guidance: [
      ...
      `This HANDS the request to the coach, which rebuilds the week from it in the background. What happens next is server policy, not yours to promise: under the default lead posture the rebuilt week lands at the next natural boundary (a Monday) with a one-tap Undo; only when the athlete has set Cairn to review everything does it wait for them to confirm. The server appends the exact receipt. So never say "done" or "updated" (nothing changes in this turn), never tell them they must confirm, and never say the change cannot be made from chat — say the coach is rebuilding it. When the same ask is already in flight, re-flagging is harmless: the server points back at that one standing request instead of building it twice. A materially different ask is flagged on its own.`,
    ],
  },
  revert_decision: {
    type: "revert_decision",
