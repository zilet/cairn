...
// clears the list.
api.get("/garmin/unreconciled", (req, res) =>
...
api.get("/export/db", async (_req, res) => {
...
api.get("/evidence/summary", (_req, res) => res.json(repo.evidenceSummary()));
...
api.get("/injury-impacts", (_req, res) => res.json(repo.getInjuryImpacts()));
...
api.post("/family", (req, res) => {
  ...
  res.json(repo.addFamily({
    ...
    allergies: b.allergies, dietary_restrictions: b.dietary_restrictions,
  }));
});
...
api.post("/supplements/understand", (req, res) => {
...
api.put("/supplements/:id", (req, res) => {
  const updated = repo.updateSupplement(Number(req.params.id), req.body ?? {});
  if (!updated) return res.status(404).json({ error: "not found" });
  res.json(updated);
});
api.delete("/supplements/:id", (req, res) => res.json(repo.deleteSupplement(Number(req.params.id))));
...
api.post("/onboard", async (req, res) => {
  const text = (req.body?.text ?? "").toString();
